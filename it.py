#!/usr/bin/env python3
"""
Zevric Email Tool - Ultra Clean Final
Owner: zevric x play | TG: @just_zevric | YT: zevricxplay
Fixed: Change Bind Email, Single YouTube button, Green UI, No extra text
"""
...
# Check Bind Info - NO extra JSON question
...
async def cbi_token_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        msg = (
            ...
            f"Countdown : {days} Day {hrs} Hour {mins} Min {secs} Sec\n"
            f"Result Code : 0"
        )
...
async def eat_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not eat_token:
            await update.message.reply_text("❌ EAT token not found.", reply_markup=sub_keyboard())
            await update.message.reply_text("Main Menu:", reply_markup=main_menu())
            return ConversationHandler.END
        ...
        if access_token:
            ...
            await update.message.reply_text(f"❌ Failed to convert. Raw EAT: {eat_token}", reply_markup=sub_keyboard())
    except Exception as e:
        await update.message.reply_text(f"Error: {e}", reply_markup=sub_keyboard())
    await update.message.reply_text("Main Menu:", reply_markup=main_menu())
    return ConversationHandler.END
...
async def bind_email_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if j.get("error") == "error_ongoing_request":
            await update.message.reply_text("❌ Pending request hai. Pehle Cancel Bind karo.", reply_markup=sub_keyboard())
            await update.message.reply_text("Main Menu:", reply_markup=main_menu())
            return ConversationHandler.END
...
async def change_tok(update: Update, context: ContextTypes.DEFAULT_TYPE):
    token = update.message.text.strip()
    context.user_data["token"] = token
    # Auto cancel pending to fix error_ongoing_request
    try:
    ...
    try:
        ...
        old = ""
    if not old:
        await update.message.reply_text("❌ No bound email found. Use Bind Email first.", reply_markup=sub_keyboard())
        await update.message.reply_text("Main Menu:", reply_markup=main_menu())
        return ConversationHandler.END
    ...
    try:
        ...
        j = r.json()
        if j.get("error") == "error_ongoing_request":
            await update.message.reply_text("❌ Still pending. Wait 1 min then try Cancel Bind.", reply_markup=sub_keyboard())
            await update.message.reply_text("Main Menu:", reply_markup=main_menu())
            return ConversationHandler.END
        await update.message.reply_text(f"✅ OTP sent to OLD email: {old}\nEnter OTP from old email:")
...
async def change_old_otp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not it:
            await update.message.reply_text(f"❌ Old OTP verify failed: {j}", reply_markup=sub_keyboard())
            await update.message.reply_text("Main Menu:", reply_markup=main_menu())
            return ConversationHandler.END
