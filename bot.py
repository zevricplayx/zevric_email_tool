...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if clean_text in ["4 CHANGE BIND EMAIL", "CHANGE BIND EMAIL", "4"]:
        ...
        await update.message.reply_text("Please select method:", reply_markup=get_method_keyboard())
        return STATE_INPUT
    if clean_text in ["5 CANCEL BIND REQUEST", "CANCEL BIND REQUEST", "Cancel Recovery Email", "5"]:
        ...
        return STATE_INPUT
    if clean_text in ["6 EAT TO ACCESS TOKEN", "EAT TO ACCESS TOKEN", "Eat-Token", "6"]:
    ...
    if clean_text in ["8 GET LOGIN HISTORY", "GET LOGIN HISTORY", "8"]:
        ...
        return STATE_INPUT
    if clean_text in ["9 CHECK BOUND ACCOUNTS", "CHECK BOUND ACCOUNTS", "Check Platform", "9"]:
    ...
    if flow == 'bind_email':
        ...
        if step == 'email':
            ...
            if res['ok']: await update.message.reply_text("OTP Sent! OTP bhejo:", reply_markup=get_youtube_keyboard()); context.user_data['step'] = 'otp'
                ...
                if res.get('captcha'): await update.message.reply_text("Captcha Block! Via Sec Code use karo.", reply_markup=get_youtube_keyboard())
        ...
        if step == 'token':
            ...
            if method == 'otp':
                ...
                if res['ok']: await update.message.reply_text(f"OTP sent to {email}\nOTP bhejo:", reply_markup=get_youtube_keyboard()); context.user_data['step'] = 'otp'
                    ...
                    if res.get('captcha'): await update.message.reply_text("Captcha! Via Sec Code use karo!", reply_markup=get_youtube_keyboard()); await update.message.reply_text("Main Menu:", reply_markup=get_reply_keyboard()); context.user_data.clear()
