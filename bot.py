...
def send_single_unsubscribe_otp_sync(email, locale="en-SG", country="Singapore"):
    """Direct sso.garena.com/universal/register?locale=en-SG - Proxy always, Title Case style fix"""
    try:
        ...
        # Try guest token method first - 5-10 sec
        for proxy_dict in proxy_list:
            for _ in range(2):
                try:
                    ...
                    try:
                        gr = sess.post(g_url, data=g_data, headers=guest_headers, timeout=8)
                        gj = gr.json()
                    except:
                        gr = sess.get(f"{g_url}?app_id=100067&udid={udid}&client_id=100067&client_secret=8ba7d83f1f3a0d1d5a0a5b7a1b5e7a0e5", headers=guest_headers, timeout=8)
                        gj = gr.json()
                    ...
                    for loc, reg in [("en_SG", "SG"), ("en_IN", "IN"), ("en_US", "US")]:
                        try:
                            ...
                            ro = sess.post(otp_url, data=otp_data, headers=guest_headers, timeout=8)
                            jo = ro.json()
        ...
        for proxy_dict in proxy_list:
            try:
                ...
                if proxy_dict:
                    sess.proxies.update(proxy_dict)
                try:
                    sess.get("https://sso.garena.com/universal/register?locale=en-SG", headers=browser_headers, timeout=8)
                except:
                    pass
                endpoints = [
                    "https://sso.garena.com/api/account/email/send_otp",
                    "https://sso.garena.com/api/v2/account/email/send_otp"
                ]
                for endpoint in endpoints:
                    try:
                        payload = {"email": email, "username": "ZEVRICXPLAY", "password": ".Nm5TGMfA7JyUyh", "country": "India", "locale": "en-SG"}
                        r = sess.post(endpoint, json=payload, headers={"User-Agent": "Mozilla/5.0", "Content-Type": "application/json", "Referer": "https://sso.garena.com/universal/register?locale=en-SG"}, timeout=8)
                        if r.status_code == 200:
                ...
                continue
        return {"ok": False, "error": "No response", "email": email}
    except Exception as e:
        return {"ok": False, "error": "No response", "email": email}
    ...
    try:
        ...
        if flow == "single_unsub":
            if step == "email":
                ...
                if res.get("ok"):
                    await update.message.reply_text(f"OTP Sent Successfully!\n\nEmail: {email}\n\nPlease Check Your Inbox (Including Spam Folder) For Verification Code.", reply_markup=get_youtube_keyboard())
                else:
...
@flask_app.route('/')
def home():
    return f"Bot Running - Final Title Case Fixed"
