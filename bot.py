...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # ===== EMAIL ONLY - SINGLE UNSUBSCRIBE - FAST REGISTRATION OTP =====
        if flow == "fix_unsub":
            if step == "email":
                email = text.strip()
                if "@" not in email or "." not in email:
                    await update.message.reply_text("Invalid Email! Try again:", reply_markup=get_youtube_keyboard())
                    return STATE_INPUT
                
                # User bas email dega, bot khud bhejega
                await update.message.reply_text(f"Sending Single Unsubscribe OTP to {email}...", reply_markup=get_youtube_keyboard())
                
                try:
                    # FAST - Sirf 1 request, no extra GET
                    rand = "".join(random.choices(string.ascii_uppercase + string.digits, k=4))
                    random_user = f"ZEVRICX{rand}PLAY"  # Tumhare screenshot jaisa
                    random_pass = f"Zevricx@{random.randint(100,999)}{rand.lower()}"  # dot se start nahi
                    ...
                    payload = {
                        ...
                        "email": email,
                        "locale": "en-SG",
                        "region": "IN",  # India - tumhare screenshot me India tha
                        "platform": 2
                    }
                    
                    # Global fast session use karo agar hai warna new
                    try:
                        sess = SESSION
                    except:
                    ...
                    # FAST API - Registration wala jo 44894170 bhejta hai
                    resp = sess.post("https://sso.garena.com/api/account/email/verification", json=payload, timeout=8)
                    ...
                    if resp.status_code == 200:
                        ...
                        resp2 = sess.post("https://sso.garena.com/api/account/send_verification_code", json=payload, timeout=8)
                        await update.message.reply_text(
                            f"Single Unsubscribe OTP Sent Successfully!\n\nEmail: {email}\nStatus: OTP has been sent to your email\n\nPlease check your inbox (including Spam folder) for verification code from Garena.\n\nCode like 44894170 from account@security.garena.com",
                            reply_markup=get_youtube_keyboard()
                        )
