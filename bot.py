...
def send_single_unsubscribe_otp_sync(email, locale="en-SG", country="Singapore"):
    """
    WEBSITE AUTOMATION as user said:
    - Go to sso.garena.com registration
    - Fill name ZEVRICXPLAY, password .Nm5TGMfA7JyUyh
    - Check gmail server, if India select India else Singapore
    - Click GET CODE
    """
    try:
        proxy_dict = get_proxy_dict()
        ...
        # Detect email server/country as user explained
        email_lower = email.lower()
        if ".in" in email_lower or "india" in country.lower():
            countries_to_try = ["India", "Singapore"]
            locales_to_try = ["en-IN", "en-SG"]
        else:
            countries_to_try = ["India", "Singapore", "Indonesia"]
            locales_to_try = ["en-IN", "en-SG", "en-ID"]
        
        username = "ZEVRICXPLAY"
        password = ".Nm5TGMfA7JyUyh"
        
        # Method 1: Guest token + game API (bypasses 404, most reliable)
        try:
            guest_res = get_guest_access_token_sync(proxy_dict)
            if guest_res.get("ok"):
                otp_res = send_otp_sync(email, guest_res["token"], retry_count=0)
                if otp_res.get("ok"):
                    return {"ok": True, "data": otp_res["data"], "email": email}
        except:
            pass
        
        # Method 2: Website registration GET CODE flow
        for idx, country_name in enumerate(countries_to_try):
            ...
            try:
                ...
                try:
                    session.get(reg_url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36"}, timeout=15)
                except:
                    pass
                ...
                for ep_url in endpoints:
                    try:
                        ...
                        r = session.post(ep_url, headers=headers, data=payload, timeout=20)
                        try:
                            j = r.json()
                        except:
                            j = {"raw": r.text[:500]}
                        if r.status_code == 200 and (j.get("result") == 0 or j.get("error") == 0):
                            return {"ok": True, "data": j, "email": email, "country": country_name}
                        txt = (str(j) + r.text).lower()
                        if "success" in txt and "sent" in txt:
                            return {"ok": True, "data": j, "email": email, "country": country_name}
                    except:
                        continue
            except:
        ...
        return {"ok": False, "error": "All methods failed", "email": email}
    except Exception as e:
