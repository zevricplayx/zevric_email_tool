...
def main_menu():
    ...
    m.add(types.KeyboardButton(f"🟢 Send Single Unsubscribe OTP"), types.KeyboardButton(f"🟢 Send Double Unsubscribe OTP"))
            ...
            try:
                ...
                if action=="unbind":
                    ...
                    if r2.json().get("result")==0:
                        ...
                        bot.send_message(chat_id, f"❌ {r2.text[:400]}", reply_markup=yt_btn())
                    del user_states[chat_id]
                    bot.send_message(chat_id, f"{bold('Main Menu - Please select an option:')} 👇", reply_markup=main_menu())
