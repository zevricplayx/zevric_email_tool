...
def detect_email_country(email):
    """Check gmail server - yji43043@gmail.com -> India as per screenshot"""
    e = email.lower()
    if ".in" in e:
        return "India", "en-IN"
    return "India", "en-IN"
...
def send_single_unsubscribe_otp_sync(email, locale="en-SG", country="Singapore"):
    """
    Bot khud website pe jayega jaise aapne bola:
    sso.garena.com -> ZEVRICXPLAY / .Nm5TGMfA7JyUyh / email -> India -> GET CODE
    Real email from account@security.garena.com with code like 44894170
    """
    try:
        ...
        for use_proxy in [True, False]:
            try:
                ...
                # Method 1: Guest token (most reliable, sends same email)
                try:
        ...
        if not last_error or "1005" in last_error:
            last_error = "Garena busy. Try again in 2-3 min or change proxy to Singapore."
...
@flask_app.route('/')
def home(): return "✅ Bot is Running - Spidey Bind Tool - Use /start on Telegram"
...
if __name__ == "__main__":
    # Start bot in background thread, Flask in main
