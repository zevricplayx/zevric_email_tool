...
async def h_token(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mapping={'bind_info': lambda: api_bind_info(tok),'unbind': lambda: api_unbind(tok),'cancel': lambda: api_cancel(tok),'revoke': lambda: api_revoke(tok),'hist': lambda: history(tok),'plat': lambda: api_bound(tok),}
    if a in mapping:
...
def build_ptb():
    ...
    conv = ConversationHandler(
        ...
        states={ASK_TOKEN: [MessageHandler(filters.TEXT & ~filters.COMMAND, h_token)], ASK_NEW_EMAIL: [MessageHandler(filters.TEXT & ~filters.COMMAND, h_new)], ASK_OLD_EMAIL: [MessageHandler(filters.TEXT & ~filters.COMMAND, h_old)], ASK_EAT: [MessageHandler(filters.TEXT & ~filters.COMMAND, h_eat)],},
        fallbacks=[CommandHandler("cancel", cancel_cmd)], allow_reentry=True)
