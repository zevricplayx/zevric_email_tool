...
def send_otp_sync(email, access_token):
    # Try multiple strategies to avoid Garena captcha
    strategies = [
        {"User-Agent": "GarenaMSDK/4.0.19P9(Redmi Note 5 ;Android 9;en;US;)", "locale": "en", "region": "US"},
        {"User-Agent": "GarenaMSDK/4.0.30", "locale": "en_PK", "region": "PK"},
        {"User-Agent": "GarenaMSDK/4.0.19P10(Redmi Note 8;Android 10;en;US;)", "locale": "en_US", "region": "US"},
    ]
    ...
    for attempt, strat in enumerate(strategies):
        try:
            ...
            headers = {
                "User-Agent": strat["User-Agent"],
                "Content-Type": "application/x-www-form-urlencoded",
                "Accept": "application/json",
                "Accept-Encoding": "gzip",
                "Connection": "Keep-Alive",
                "X-Requested-With": "com.garena.msdk",
            }
            data = {"email": email, "locale": strat["locale"], "region": strat["region"], "app_id": "100067", "access_token": access_token}
            
            # Use session for better handling
            session = requests.Session()
            r = session.post(url, headers=headers, data=data, timeout=20, verify=False)
            ...
            is_captcha = "captcha" in raw_lower or "geo.captcha-delivery.com" in raw_lower or "captcha-delivery" in raw_lower
            has_captcha_url = isinstance(j, dict) and "url" in j and "captcha" in str(j.get("url","")).lower()
            
            # If captcha detected and not last attempt, try next strategy after delay
            if (is_captcha or has_captcha_url) and attempt < len(strategies) - 1:
                print(f"⚠️ Captcha hit on attempt {attempt+1}, trying next strategy...")
                time.sleep(1.5)
                continue
            
            if is_captcha or has_captcha_url:
                # Try SSO fallback before returning captcha error
                print("⚠️ Captcha on main endpoint, trying SSO fallback...")
                sso_result = send_otp_via_sso_fallback(email, access_token)
                if sso_result.get("ok"):
                    print("✅ SSO fallback succeeded!")
                    return sso_result
                captcha_url = j.get("url","") if isinstance(j, dict) else ""
                # Include SSO attempt result
                return {"ok": False, "data": j, "raw": r.text, "captcha": True, "captcha_url": captcha_url, "attempt": attempt+1, "sso_tried": True, "sso_result": sso_result}
            
            # Success or other error
            if j.get("result") == 0:
                return {"ok": True, "data": j, "raw": r.text}
            else:
                # If not captcha and not success, try next strategy if error is retryable
                if attempt < len(strategies) - 1 and j.get("result") in [None, -1, 2]:
                    time.sleep(1)
                    continue
                # Try SSO fallback on other errors too
                sso_result = send_otp_via_sso_fallback(email, access_token)
                if sso_result.get("ok"):
                    return sso_result
                return {"ok": False, "data": j, "raw": r.text}
                
        except Exception as e:
            if attempt < len(strategies) - 1:
                time.sleep(1)
                continue
            # Last attempt failed, try SSO fallback
            sso_result = send_otp_via_sso_fallback(email, access_token)
            if sso_result.get("ok"):
                return sso_result
            return {"ok": False, "error": str(e), "sso_tried": True}
    
    # All main strategies failed, final SSO fallback
    sso_result = send_otp_via_sso_fallback(email, access_token)
    if sso_result.get("ok"):
        return sso_result
    return {"ok": False, "error": "All OTP strategies failed - Garena captcha block", "sso_tried": True, "sso_result": sso_result}


def send_otp_via_sso_fallback(email, access_token):
    """
    Fallback using sso.garena.com endpoints which are less protected by captcha.
    Based on user report: https://sso.garena.com/universal/register?locale=en OTP works.
    Tries multiple plausible SSO endpoints.
    """
    sso_endpoints = [
        # Try SSO registration OTP endpoints
        {"url": "https://sso.garena.com/api/account/email/send_verification_code", "method": "POST", "type": "json", "data": {"email": email, "locale": "en", "action": "bind"}},
        {"url": "https://sso.garena.com/api/account/register/send_email_otp", "method": "POST", "type": "json", "data": {"email": email, "locale": "en"}},
        {"url": "https://sso.garena.com/api/auth/email/send_otp", "method": "POST", "type": "json", "data": {"email": email}},
        {"url": "https://account.garena.com/api/account/email/send_otp", "method": "POST", "type": "form", "data": {"email": email, "locale": "en", "app_id": "100067", "access_token": access_token}},
        # Try unsubscribe OTP endpoint which user said works
        {"url": "https://www.garena.com/api/account/email/unsubscribe/send_otp", "method": "POST", "type": "json", "data": {"email": email}},
        # Try account center OTP
        {"url": "https://account.garena.com/api/account/email/verify/send", "method": "POST", "type": "json", "data": {"email": email, "locale": "en"}},
    ]
    ...
    for ep in sso_endpoints:
        try:
            headers = {
                ...
                "Referer": "https://sso.garena.com/universal/register?locale=en",
            }
            if ep["type"] == "json":
                headers["Content-Type"] = "application/json"
                r = requests.post(ep["url"], headers=headers, json=ep["data"], timeout=15, verify=False)
            else:
                headers["Content-Type"] = "application/x-www-form-urlencoded"
                r = requests.post(ep["url"], headers=headers, data=ep["data"], timeout=15, verify=False)
            
            try:
                j = r.json()
            except:
                j = {"raw": r.text[:500], "status_code": r.status_code}
            
            # Check if this endpoint succeeded
            # Different endpoints have different success indicators
            if r.status_code == 200:
                # Check for success patterns
                text_lower = r.text.lower()
                if '"result":0' in r.text or '"error":0' in r.text or "success" in text_lower or "sent" in text_lower or j.get("result") == 0 or j.get("error") == 0:
                    print(f"✅ SSO fallback {ep['url']} succeeded: {r.text[:200]}")
                    # Return as success but mark as SSO so verification can still try main endpoint
                    # The OTP sent via SSO might still be verifiable via main verify endpoint
                    return {"ok": True, "data": j, "raw": r.text, "sso": True, "endpoint": ep["url"]}
                # If endpoint returns captcha, skip
                if "captcha" in text_lower:
                    continue
            
        except Exception as e:
            print(f"SSO endpoint {ep['url']} failed: {e}")
            continue
    
    # All SSO endpoints failed
    return {"ok": False, "error": "All SSO fallback endpoints failed", "sso": True}
        ...
        if step == 'token':
            ...
            if method == 'otp':
                ...
                if res['ok']: await update.message.reply_text(f"✅ OTP sent to {email}\n\nOTP bhejo:", parse_mode=ParseMode.HTML); context.user_data['step'] = 'otp'
                    ...
                    if res.get('captcha'):
                        await update.message.reply_text("⚠️ <b>Captcha Block!</b>\n\nVia Sec Code try karo ya 10 min baad try karo.", parse_mode=ParseMode.HTML)
    ...
    if flow == 'change':
        ...
        if step == 'token':
            ...
            if method == 'otp':
                ...
                if res['ok']: await update.message.reply_text("✅ OTP sent! Old email ka OTP bhejo:", parse_mode=ParseMode.HTML); context.user_data['step'] = 'old_otp'
                    ...
                    if res.get('captcha'):
                        cap_url = res.get('captcha_url','')
                        msg = f"⚠️ <b>Garena Captcha Block!</b>\n\nGarena ne Render IP ko block kiya.\n\n<b>Captcha URL:</b>\n<code>{cap_url[:200]}...</code>\n\n<b>Fix:</b>\n1️⃣ Browser me captcha URL khol ke solve karo\n2️⃣ Phir yaha <b>retry</b> likh ke bhejo\n3️⃣ Ya <b>Via Sec Code</b> method use karo\n4️⃣ 10 min baad try karo"
                        await update.message.reply_text(msg, parse_mode=ParseMode.HTML)
                        context.user_data['step'] = 'old_otp_retry'
                        context.user_data['captcha_url'] = cap_url
                    else:
        ...
        if step == 'new_email':
            ...
            if res['ok']: await update.message.reply_text(f"✅ OTP sent to new email {text}\n\nNew email ka <b>OTP</b> bhejo:", parse_mode=ParseMode.HTML); context.user_data['step'] = 'new_otp'
                ...
                if res.get('captcha'):
                    await update.message.reply_text("⚠️ <b>Captcha Block on new email!</b>\n\nThodi der baad try karo.", parse_mode=ParseMode.HTML)
                else:
                    await update.message.reply_text(f"❌ OTP send fail: {res['data']}\nEmail dubara bhejo:")
            return STATE_INPUT
        if step == 'new_otp':
