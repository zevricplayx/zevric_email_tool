...
# ===== FORCE JOIN VERIFICATION - 5 CHANNELS =====
REQUIRED_CHANNELS = [
    {"name": "Zevric All Update", "link": "https://t.me/zevric_all_update", "username": "@zevric_all_update", "chat_id": "@zevric_all_update"},
    {"name": "Zevric X Play", "link": "https://t.me/zevricxplay", "username": "@zevricxplay", "chat_id": "@zevricxplay"},
    {"name": "Zevric Banner", "link": "https://t.me/zevricbaner", "username": "@zevricbaner", "chat_id": "@zevricbaner"},
    {"name": "Zevric Api Tools", "link": "https://t.me/zevric_api_tools", "username": "@zevric_api_tools", "chat_id": "@zevric_api_tools"},
    {"name": "Zevric Illegal Vounch", "link": "https://t.me/zevric_illigalvounch", "username": "@zevric_illigalvounch", "chat_id": "@zevric_illigalvounch"},
]
...
async def check_user_joined_all(context, user_id):
    ...
    for ch in REQUIRED_CHANNELS:
        try:
            ...
            # If user is left, kicked, or not member
            if member.status in ["left", "kicked"]:
                not_joined.append(ch)
        except Exception as e:
            # If bot is not admin or channel not found, try to assume not joined or ignore?
            # For safety, if we can't check, we allow (or you can make it strict)
            # Here we log and treat as joined to avoid blocking if bot not admin yet
            print(f"Check join error for {ch['name']}: {e}")
            # Uncomment next line to make strict:
            # not_joined.append(ch)
            pass
    return not_joined

def get_force_join_keyboard():
    """Blue join buttons + Green verify button like screenshot"""
    keyboard = []
    for ch in REQUIRED_CHANNELS:
        # Blue button with join link
        try:
            if HAS_STYLE:
...
# Normal keyboard - 10 options as in app.py file, plus 2 unsubscribe (total 12 as before but now normal text)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Verified - show welcome like screenshot 2
    ...
    try:
        ...
        if data == "check_join":
            ...
            if not_joined:
                ...
                await context.bot.send_message(chat_id=query.message.chat_id, text="Main Menu - Please select an option:", reply_markup=get_reply_keyboard())
        return STATE_INPUT
    except Exception as e:
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        # Force join check on every message - if left, ask verification again
        user_id = update.effective_user.id
        not_joined = await check_user_joined_all(context, user_id)
        if not_joined:
            await update.message.reply_text(get_force_join_text(not_joined), reply_markup=get_force_join_keyboard())
        ...
        if text_lower == "check bind info" or text_lower == "check recovery email":
            context.user_data.clear()
        ...
        if text_lower == "check bound accounts":
            context.user_data.clear()
        ...
        if text_lower == "back to menu" or text_lower == "back":
            context.user_data.clear()
            await update.message.reply_text("Main Menu", reply_markup=get_reply_keyboard())
            return STATE_INPUT
        ...
        if flow == "bind_info":
            if step == "token":
                ...
                if not bind["ok"]:
                    ...
                    # Show BOTH current and pending as requested
                    current_display = email if email else "None"
                    if email_to_be:
                        pending_display = email_to_be
                        msg = f"Email Information\n\nCurrent Email: {current_display}\nPending Email: {pending_display}\nConfirm In: {countdown_human}"
                    else:
        ...
        if flow == "unbind":
            if step == "method":
                if "otp" in text_lower or "email" in text_lower:
...
@flask_app.route('/')
def home():
    return f"Bot Running - Normal Text - Options As In File"
