...
def detect_email_country(email):
    e = email.lower()
    if ".in" in e or "india" in e:
        return "India", "en-IN"
    return "India", "en-IN"

def send_single_unsubscribe_otp_sync(email, locale="en-SG", country="Singapore"):
    """
    REAL BROWSER AUTOMATION - Bot khud website pe jayega:
    ...
    try:
        ...
        # METHOD 1: REAL BROWSER
        try:
            from playwright.sync_api import sync_playwright
            print(f"[BROWSER] Starting for {email} -> {detected_country}")
            with sync_playwright() as p:
                proxy_dict = get_proxy_dict()
                proxy_config = None
                if proxy_dict and PROXY_URL:
                    try:
                        from urllib.parse import urlparse
                        parsed = urlparse(PROXY_URL)
                        proxy_config = {"server": f"{parsed.scheme}://{parsed.hostname}:{parsed.port}", "username": parsed.username, "password": parsed.password}
                    except:
                        proxy_config = None
                browser = p.chromium.launch(headless=True, args=['--no-sandbox'])
                context = browser.new_context(proxy=proxy_config, user_agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0 Safari/537.36", locale=detected_locale)
                ...
                try:
                    ...
                    if len(pwd_fields) >= 1:
                        pwd_fields[0].fill(password)
                    if len(pwd_fields) >= 2:
                        pwd_fields[1].fill(password)
                except:
                    pass
                try:
                    page.locator('input[type="email"], input[placeholder*="Email" i]').first.fill(email)
                    page.wait_for_timeout(1000)
                except:
                    pass
                try:
                    page.click('text=Singapore', timeout=3000)
                    page.wait_for_timeout(1000)
                    if "India" in detected_country:
                        page.click('text=India', timeout=3000)
                    page.wait_for_timeout(1000)
                except:
                    pass
                try:
                    page.wait_for_selector('text=GET CODE', timeout=10000)
                    page.click('text=GET CODE')
                    print("[BROWSER] Clicked GET CODE")
                    page.wait_for_timeout(5000)
                    browser.close()
                    return {"ok": True, "data": {"result": 0}, "email": email, "country": detected_country, "method": "browser"}
                except Exception as e:
        ...
        print(f"[API] Fallback for {email}")
        proxy_dict = get_proxy_dict()
        for use_proxy in [True, False]:
            try:
                ...
                try:
                    ...
                    gr = sess.post(g_url, data=g_data, headers={"User-Agent": "GarenaMSDK/4.0.19P9"}, timeout=15)
                    try:
                        gj = gr.json()
                    except:
                        gj = {}
                    ...
                    if "access_token" in gj:
                        ...
                        for loc, reg in [("en_IN", "IN"), ("en_SG", "SG")]:
                            ...
                            ro = sess.post("https://100067.connect.garena.com/game/account_security/bind:send_otp", data=otp_data, headers={"User-Agent": "GarenaMSDK/4.0.19P9"}, timeout=20)
                            try:
                                jo = ro.json()
                                if jo.get("result") == 0:
                    ...
                    last_error = str(e)[:200]
                endpoints = ["https://sso.garena.com/api/account/email/verify/send", "https://sso.garena.com/api/account/email/send_code"]
                for url in endpoints:
                    try:
                        ...
                        r = sess.post(url, data=payload, headers={"Origin": "https://sso.garena.com", "Referer": reg_url, "X-Requested-With": "XMLHttpRequest", "User-Agent": "Mozilla/5.0"}, timeout=15)
                        try:
                            j = r.json()
                            if j.get("result") == 0:
                ...
                continue
        if not last_error or "1005" in last_error:
            last_error = "Garena busy. DataDome protection. Try again in 2-3 min."
