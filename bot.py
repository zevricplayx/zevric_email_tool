...
def send_single_unsubscribe_otp_sync(email, locale="en-SG", country="Singapore"):
    try:
        ...
        # Try 5 times with proxy, 2 times without
        for use_proxy in [True, True, True, False, False]:
            for attempt in range(3):
                try:
                    proxy_dict = None
                    if use_proxy and PROXY_URL_LOCAL and len(PROXY_URL_LOCAL) > 10:
                        proxy_dict = {"http": PROXY_URL_LOCAL, "https": PROXY_URL_LOCAL}
                    ...
                    # Step 1: Get cookies from SSO page to bypass DataDome
                    try:
                        sess.get(
                    ...
                    try:
                        gj = gr.json() if gr else {}
                    except:
                        try:
                            gj = json.loads(gr.text) if gr else {}
                        except:
                            gj = {}
                    ...
                    if "access_token" not in gj:
                        # Try GET method
                        try:
                            gr2 = sess.get(f"{g_url}?app_id=100067&udid={udid}&client_id=100067&client_secret=8ba7d83f1f3a0d1d5a0a5b7a1b5e7a0e5", headers=headers_g, timeout=15)
                            gj = gr2.json()
                        except:
                            gj = {}
                    ...
                    if "access_token" in gj:
                        ...
                        # Step 3: Send OTP with different locales
                        for loc, reg in [("en_SG", "SG"), ("en_IN", "IN"), ("en_US", "US"), ("en", "SG")]:
                            try:
                                ...
                                ro = sess.post(otp_url, data=otp_data, headers=headers_g, timeout=20)
                                try:
                                    jo = ro.json()
                                    if jo.get("result") == 0:
                                        return {"ok": True, "data": jo, "email": email, "country": "India"}
                                    # Check for success codes
                                    if jo.get("result") in [0, 1]:
                                        return {"ok": True, "data": jo, "email": email, "country": "India"}
                                    last_error = str(jo)
                                    # If captcha or too many requests, try next locale/proxy
                                    if "1002" in last_error or "captcha" in last_error.lower() or "too_many" in last_error.lower():
                                        time.sleep(1)
                                        continue
                                except:
                                    txt = ro.text[:500] if hasattr(ro, 'text') else ""
                                    last_error = txt
                                    if "0" in txt and "result" in txt.lower():
                                        # Sometimes result 0 in text but json parse failed
                                        if '"result":0' in txt or '"result": 0' in txt:
                                            return {"ok": True, "data": {"result": 0}, "email": email, "country": "India"}
                            except Exception as e:
                                last_error = str(e)[:300]
                                continue
                        # If all locales failed, try next proxy/attempt
                    else:
                        last_error = str(gj)[:300] if gj else "Guest token failed"
        ...
        if "1005" in last_error or "datadome" in last_error.lower() or "captcha" in last_error.lower() or "Unknown" in last_error:
            last_error = "Garena DataDome active - Singapore proxy is trying to bypass. Please wait 2-3 min and try again. If still fails, check proxy: " + PROXY_URL_LOCAL[:30] + "..."
                ...
                try:
                    ...
                    if res.get("ok"):
                        await update.message.reply_text(
                            ...
                            f"📬 **Please check your inbox** (including Spam folder) for verification code from Garena.\n"
                            f"📧 From: `account@security.garena.com`\n"
                            f"🔢 Code like: `44894170`\n"
                            f"⏰ Expire: 10 minutes\n\n"
                            f"💡 If not received, wait 2-3 min and try again with Singapore proxy.",
