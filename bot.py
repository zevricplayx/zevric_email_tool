...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    clean_text = text
    # Normalize - handle newlines, extra spaces, case insensitive for robust matching
    norm = clean_text.replace("\n", " ").replace("\r", " ").upper()
    norm = " ".join(norm.split())
    
    flow = context.user_data.get('flow'); step = context.user_data.get('step')

    # === SINGLE UNSUBSCRIBE OTP - MUST BE AT TOP - Fix for direct Main Menu issue ===
    ...
    if flow == 'single_unsubscribe':
        if step == 'email':
            ...
            await update.message.reply_text(f"Sending Single Unsubscribe OTP to\n{email}...", reply_markup=get_youtube_keyboard())
            # Send OTP via SSO Garena - server according to email locale
            # Default to Singapore (en-SG) as shown in screenshot, but also try India
            res = send_single_unsubscribe_otp_sync(email, locale="en-SG", country="Singapore")
            if res['ok']:
                msg = f"Single Unsubscribe OTP Sent Successfully!\n\nEmail: {email}\nStatus: OTP has been sent to your email\n\nPlease check your inbox (including Spam folder) for verification code from Garena."
                await update.message.reply_text(msg, reply_markup=get_youtube_keyboard())
            else:
                await update.message.reply_text(f"Failed to send OTP: {res.get('error', 'Unknown error')}\n\nTry again or use different email.", reply_markup=get_youtube_keyboard())
            await update.message.reply_text("Main Menu - Please select an option:", reply_markup=get_reply_keyboard())
            context.user_data.clear()
            return STATE_INPUT
