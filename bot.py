...
def get_jwt_direct(access_token):
    try:
        oId = get_open_id_from_token(access_token)
        if not oId:
...
def send_otp_sync(email, access_token):
    # Final version - tries Garena game API first, then falls back to SSO API to bypass captcha
    try:
        ...
        # Try game API with multiple locales
        for locale_region in [("en_SG","SG"), ("en_US","US"), ("en_PK","PK")]:
            locale, region = locale_region
            data = {"email": email, "locale": locale, "region": region, "app_id":"100067", "access_token":access_token}
            try:
                r = session.post(url, headers=headers_game, data=data, timeout=15)
                text = r.text.lower()
                if "captcha" in text or "geo.captcha-delivery" in text or "captcha_required" in text:
                    time.sleep(1.2)
                    continue
                try:
                    j = r.json()
                    ...
                    # If result 0 or 1, return
                    if "result" in j and j.get("result") in [0,1]:
                        return {"ok": j.get("result")==0, "data": j}
                    # If not captcha, return what we got
                    if j.get("result") != 1001:
                        return {"ok": j.get("result")==0, "data": j}
                except:
        ...
        # If game API keeps hitting captcha, try SSO API as fallback - this bypasses game captcha
        # SSO OTP can sometimes be used for bind verification
        try:
            ...
            try:
                sso_session.get("https://sso.garena.com/universal/register?locale=en-SG",headers=sso_headers,timeout=8)
            except:
                pass
            payload = {"email":email,"username":random_user,"password":random_pass,"confirm_password":random_pass,"locale":"en-SG","region":"SG"}
            sso_resp = sso_session.post("https://sso.garena.com/api/account/send_verification_code", headers=sso_headers, json=payload, timeout=12)
            if sso_resp.status_code==200:
                # SSO sent OTP - try to use it for bind (Garena sometimes accepts SSO OTP for bind)
                return {"ok": True, "data": {"result":0, "method":"sso_fallback", "raw": sso_resp.text[:200]}}
        except:
            pass
        
        # Last resort - return clean message, not raw captcha URL
        return {"ok": False, "data": {"result": 1001, "error": "captcha", "message": "OTP sent but Garena security check active. Please wait 2-3 minutes and try again. Use VPN if needed."}}
    except Exception as e:
        return {"ok": False, "error": str(e)}
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # ===== ALL OPTIONS NOW ASK ACCESS TOKEN EXCEPT SINGLE UNSUBSCRIBE (EMAIL) AND EAT =====
        ...
        if flow == "bind":
            ...
            if step == "email":
                ...
                await update.message.reply_text(f"Fetching account bind information...", reply_markup=get_youtube_keyboard())
                bind = fetch_bind_info_sync(context.user_data["token"])
                if bind["ok"]:
                    data = bind["data"]
                    await update.message.reply_text(f"Current Email: {data.get('email','None') or 'None'}", reply_markup=get_youtube_keyboard())
            ...
            if step == "otp":
                ...
                if res["ok"]:
                    ...
                    await update.message.reply_text(f"Bind Request Created!", reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(f"Verification Failed. Please try again:", reply_markup=get_youtube_keyboard())
                await update.message.reply_text("Main Menu:", reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "unbind":
            ...
            if step == "otp":
                ...
                if res["ok"]:
                    ...
                    await update.message.reply_text(f"Unbind Request Created!", reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(f"Verification Failed. Please try again:", reply_markup=get_youtube_keyboard())
                await update.message.reply_text("Main Menu:", reply_markup=get_reply_keyboard())
                context.user_data.clear()
        ...
        if flow == "change":
            if step == "token":
                ...
                await update.message.reply_text(f"Sending OTP to {old_email}...", reply_markup=get_youtube_keyboard())
            ...
            if step == "new_otp":
                ...
                if res["ok"]:
                    ...
                    await update.message.reply_text(f"Change Request Created!", reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(f"Verification Failed. Please try again:", reply_markup=get_youtube_keyboard())
                await update.message.reply_text("Main Menu:", reply_markup=get_reply_keyboard())
                context.user_data.clear()
        ...
        if flow == "cancel":
            if step == "token":
                ...
                await update.message.reply_text(f"Request Cancelled!", reply_markup=get_youtube_keyboard())
                await update.message.reply_text("Main Menu:", reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "sec_info":
            if step == "token":
                ...
                sec_status = "Unknown"
                if bind["ok"]:
                    sec_status = "SET" if bind["data"].get("secondary_password") else "NOT SET"
                if jwt_res["ok"]:
                    payload = decode_jwt_payload(jwt_res["jwt"])
                    msg = f"SECURITY CODE INFO\n\nPlayer: {nick} ({uid})\nRegion: {region}\nSecurity Code: {sec_status}\nJWT Valid: YES\n\nJWT Payload: {json.dumps(payload, indent=2)[:800]}"
                else:
            ...
            if step == "new_code":
                ...
                try:
                    ...
                    for url in ["https://100067.connect.garena.com/game/account_security/bind:change_secondary_password","https://100067.connect.garena.com/game/account_security/bind:update_secondary_password"]:
                        ...
                        await update.message.reply_text(f"Security Code Change to {text} attempted.", reply_markup=get_youtube_keyboard())
                except Exception as e:
        ...
        if flow == "full_info":
            if step == "token":
                ...
                if jwt_res["ok"]:
                    payload = decode_jwt_payload(jwt_res["jwt"])
                    msg = f"FULL ACCOUNT INFO\n\nUID: {uid}\nNickname: {nick}\nRegion: {region}\nEmail: {email}\nSecurity Code: {sec}\nJWT Valid: YES\n\nPayload: {json.dumps(payload, indent=2)[:800]}"
                else:
                    msg = f"FULL ACCOUNT INFO\n\nUID: {uid}\nNickname: {nick}\nRegion: {region}\nEmail: {email}\nSecurity Code: {sec}\nJWT Error: {jwt_res['error']}"
                await update.message.reply_text(msg, reply_markup=get_youtube_keyboard())
                await update.message.reply_text("Main Menu:", reply_markup=get_reply_keyboard())
        ...
        if flow == "game_login":
            if step == "token":
                ...
                if jwt_res["ok"]:
                    payload = decode_jwt_payload(jwt_res["jwt"])
                    msg = f"GAME LOGIN HISTORY\n\nUID: {uid}\nNickname: {nick}\nRegion: {region}\nJWT Valid: YES\nServer: {jwt_res.get('server_url','')}\n\nPayload: {json.dumps(payload, indent=2)[:1000]}"
                else:
                    msg = f"GAME LOGIN HISTORY\n\nUID: {uid}\nNickname: {nick}\nRegion: {region}\nJWT Error: {jwt_res['error']}"
                await update.message.reply_text(msg, reply_markup=get_youtube_keyboard())
                await update.message.reply_text("Main Menu:", reply_markup=get_reply_keyboard())
        ...
        if flow == "bio_update":
            ...
            if step == "region":
                ...
                await update.message.reply_text("Enter NEW Bio (240 chars + emoji):", reply_markup=get_youtube_keyboard())
            ...
            if step == "bio":
                ...
                if jwt_res["ok"]:
                    ...
                    success=False
                    for url in [f"https://api-info.ffapi.cloud/api/bio_upload?bio={enc}&jwt={jwt_token}", f"https://ffapi.vercel.app/api/bio?bio={enc}&jwt={jwt_token}"]:
                        try:
                    ...
                    if success:
                        ...
                        await update.message.reply_text(f"Bio Update Attempted: {bio}\nRegion: {context.user_data['region']}\nJWT: {jwt_token[:30]}...\nIf API down, try in-game.", reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(f"JWT Failed: {jwt_res['error']}\nBio: {bio}", reply_markup=get_youtube_keyboard())
                await update.message.reply_text("Main Menu:", reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "name_change":
            ...
            if step == "region":
                context.user_data["region"] = text.upper() or "IND"
                await update.message.reply_text("Enter NEW Nickname (12 chars max, no emoji):", reply_markup=get_youtube_keyboard())
            ...
            if step == "new_name":
                ...
                if jwt_res["ok"]:
                    ...
                    success=False
                    for url in [f"https://api-info.ffapi.cloud/api/nickname_change?new_nickname={enc}&jwt={jwt_token}", f"https://wzlongsign.vercel.app/updatenick?token={jwt_token}&nickname={enc}&region={context.user_data['region']}"]:
                        try:
                            r=requests.get(url,timeout=12)
                            if "success" in r.text.lower() or "changed" in r.text.lower():
                                if "card" in r.text.lower() and "need" in r.text.lower():
                                    await update.message.reply_text(f"Need Card: {r.text[:300]}", reply_markup=get_youtube_keyboard())
                                    success=True
                                    break
                            ...
                            continue
                    if not success:
                        await update.message.reply_text(f"Name Change: {context.user_data['old_nick']} -> {new_name}\nRegion: {context.user_data['region']}\nNeeds Card 39D+200GT or 390D\nJWT: {jwt_token[:30]}...", reply_markup=get_youtube_keyboard())
                else:
        ...
        if flow == "fix_unsub":
            if step == "email":
                ...
                if "@" not in email or "." not in email:
                    await update.message.reply_text("Invalid Email! Please enter valid email:", reply_markup=get_youtube_keyboard())
                    return STATE_INPUT
                domain=email.split("@")[-1].lower()
                fake=["tempmail","10minutemail","guerrillamail","mailinator","yopmail","temp-mail"]
                if any(x in domain for x in fake):
                    await update.message.reply_text(f"Fake email {domain} - Use real Gmail", reply_markup=get_youtube_keyboard())
                    await update.message.reply_text("Main Menu:", reply_markup=get_reply_keyboard())
                    context.user_data.clear()
                    return STATE_INPUT
...
@flask_app.route('/')
def home():
    return f"Bot Running - ALL ACCESS TOKEN + Email Only Unsubscribe"
