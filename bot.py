...
async def change_confirm_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await http_post(send_otp_url, data=data, headers=headers)
        await update.message.reply_text("OTP sent to old email. Enter OTP from old email:", reply_markup=ForceReply(selective=True))
    except Exception as e:
...
async def change_new_email_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await http_post(send_otp_url, data=data, headers=headers)
        await update.message.reply_text("OTP sent to new email. Enter OTP from new email:", reply_markup=ForceReply(selective=True))
    except Exception as e:
...
async def change_otp_new_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            await update.message.reply_text(f"Rebind response: {r.json()}")
        except Exception:
...
def main():
    ...
    conv_cbi = ConversationHandler(
        ...
        states={
            ...
            CBI_SHOW_RAW: [MessageHandler(filters.Regex("^(yes|no)$", flags=0) | filters.TEXT, cbi_show_raw_choice)]
        },
