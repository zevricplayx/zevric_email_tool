#!/usr/bin/env python3
"""
ULTRA PREMIUM FIXED - Full fancy font everywhere + keyboard fancy + options work
Fixes: Regex broke with fancy keyboard -> Now custom FancyButtonFilter with unfancy()
Every text via fancy()
"""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = fancy(f"Welcome, {user_name}!") + f"\n\n" + fancy("Welcome to Garena Email Tool") + f"\n\nOwner: zevric x play\nTelegram: @just_zevric\nYouTube: zevricxplay"
    await update.message.reply_text(welcome, reply_markup=main_menu())
    await update.message.reply_text(f"📢 {fancy('Hello')} {user_name}!", reply_markup=sub_keyboard())
...
async def cbi_token_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # All labels fancy, values normal
...
async def bind_otp_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not vt:
            await update.message.reply_text(f"❌ {fancy('Verify failed')}: {r.json()}", reply_markup=sub_keyboard())
            await update.message.reply_text(fancy("Main Menu:"), reply_markup=main_menu())
            return ConversationHandler.END
