...
def send_otp_sync(email, access_token):
    # Improved version with captcha avoidance - uses realistic headers and session
    try:
        ...
        # Try multiple locales to avoid captcha - SG and US are less flagged
        for locale_region in [("en_SG","SG"), ("en_US","US"), ("en_PK","PK")]:
            ...
            try:
                ...
                text = r.text
                # Check for captcha
                if "captcha" in text.lower() or "geo.captcha-delivery" in text.lower():
                    # Captcha triggered - wait and retry with different locale
                    time.sleep(1)
                    continue
                try:
                    j = r.json()
                except:
                    # If not JSON, might be captcha page
                    if "captcha" in text.lower():
                        continue
                    return {"ok": False, "data": {"raw": text[:500]}}
                # Success or known error
                if j.get("result")==0 or j.get("result")==1 or "verifier" in str(j).lower():
                    return {"ok": j.get("result")==0, "data": j}
                # If result !=0 but not captcha, return
                if "result" in j:
                    return {"ok": j.get("result")==0, "data": j}
            except:
                continue
        # If all retries hit captcha, return captcha error with helpful message
        return {"ok": False, "data": {"result": 1001, "error": "captcha_required", "message": "Garena captcha triggered. Please wait 5-10 mins or try with different IP/VPN"}}
    except Exception as e:
        return {"ok": False, "error": str(e)}
