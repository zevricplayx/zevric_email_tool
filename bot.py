...
# Exact rao.py logic - 1:1 replication, no captcha block
async def send_otp_smart(email, token, player_region):
    # rao.py always uses PK + en_PK, we will try IND first then PK like deep research
    # But we will NOT block on captcha, we will return success and let OTP arrive
    regions_to_try=["PK", "IND", player_region]
    locales={"PK":"en_PK","IND":"en_US","US":"en_US"}
    tried=set()
    last=None
    for region in regions_to_try:
        if not region or region=="Unknown" or region in tried:
            continue
        tried.add(region)
        ...
        try:
            ...
            j=r.json()
            # rao.py format_response checks result==0 = SUCCESS even if url captcha present
            # So we treat result 0 as success regardless of url
            if j.get("result")==0:
                return {"result":0,"region":region,"locale":locale,"raw":j}
            # If captcha url present but result 0, rao.py still says success, so we also treat as success
            if "url" in j:
                # Even with captcha url, rao.py still shows success if result 0, otherwise it shows failed
                # We will return whatever, but not block
                last={"result":j.get("result", -1),"region":region,"locale":locale,"raw":j, "is_captcha": "captcha" in str(j.get("url","")).lower()}
                if j.get("result")==0:
                    return last
            last=j
        except Exception as e:
            last={"error":str(e), "result":-1}
    # Return last attempt even if failed, so flow continues like rao.py
    if last is None:
        last={"result":-1}
    # Normalize to have region key
    if "region" not in last:
        last["region"]=player_region
    return last
...
async def start(update, context):
    ...
    # Exact requirement: Welcome, {name}! then Welcome to Garena Email Tool, Bot, Owner, Telegram - no YouTube text here
...
async def change_tok(update, context):
    ...
    if cd>0:
        days,rem=divmod(cd,86400); hrs,rem=divmod(rem,3600); mins,secs=divmod(rem,60)
        await update.message.reply_text(fancy(f"Countdown running {days}D {hrs}H {mins}M. Use Cancel Bind first."), reply_markup=sub_keyboard())
        await update.message.reply_text(fancy("Main Menu:"), reply_markup=main_menu())
        return ConversationHandler.END
    if not old_email:
    ...
    await update.message.reply_text(fancy(f"Sending OTP to OLD email {old_email}..."), reply_markup=sub_keyboard())
    res=await send_otp_smart(old_email, token, player_region)
    # rao.py shows SUCCESS even if captcha url present, so we always ask for OTP like rao.py
    raw=res.get("raw", res)
    if res.get("result")==0 or raw.get("result")==0:
        await update.message.reply_text(fancy(f"OTP Sent To OLD Email {old_email} via {res.get('region')} PK. Check Primary inbox.")+"\n\n"+fancy("Please Enter OTP From Old Email:"), reply_markup=sub_keyboard())
    else:
        # Even if failed, rao.py still asks for OTP, we do same but show error
        await update.message.reply_text(f"{fancy('OTP send response')}: {raw}\n\n"+fancy(f"Trying anyway, check Primary for OTP from {old_email}. If not received, use Cancel Bind and wait 10 min.")+"\n\n"+fancy("Please Enter OTP From Old Email:"), reply_markup=sub_keyboard())
