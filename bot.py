    ...
    try:
        ...
        for use_proxy in [True, False]:
            try:
                ...
                # Guest token method - most reliable, sends same email
                try:
        ...
        if not last_error or "1005" in last_error or "error_not_found" in last_error.lower():
            last_error = "Garena API busy. Try again in 2-3 min or change to Singapore proxy."
...
def home():
    ...
    return f"Bot Running - Token: {token_status} - Owner @just_zevric"
...
def run_bot():
    ...
    try:
        ...
        print(f"✅ Bot starting - Token set, Proxy: {'ON' if PROXY_URL else 'OFF'}")
