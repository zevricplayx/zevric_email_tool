        ...
        for use_proxy in [True, False]:
            for attempt in range(2):
                try:
                    ...
                    try:
                        sess.get("https://sso.garena.com/universal/register?locale=en-SG", headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}, timeout=12)
                        time.sleep(0.5)
                    except:
                        pass
                    ...
                    if "access_token" in gj:
                        ...
                        for loc, reg in [("en_IN", "IN"), ("en_SG", "SG"), ("en_US", "US")]:
                            try:
                                ...
                                headers_otp = {"User-Agent": "GarenaMSDK/4.0.19P9(Realme RMX1921 ;Android 11;en;US;)", "Content-Type": "application/x-www-form-urlencoded"}
                    ...
                    continue
        return {"ok": False, "error": "Garena DataDome protection active. Try again in 2-3 minutes or contact @just_zevric", "email": email}
    except Exception as e:
...
def send_otp_sync(email, access_token):
    ...
    for attempt, strat in enumerate(strategies):
        try:
            ...
            if is_captcha or has_captcha_url:
                ...
                return {"ok": False, "data": j, "raw": r.text, "captcha": True, "captcha_url": captcha_url, "attempt": attempt+1}
    ...
    return {"ok": False, "error": "All OTP strategies failed - Garena captcha block"}
...
def get_youtube_keyboard():
    keyboard = [[InlineKeyboardButton("📺 Subscribe YouTube Channel", url=YOUTUBE_LINK)]]
