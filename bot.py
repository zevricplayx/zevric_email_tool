...
def force_join_markup():
    # As per your photo: Blue Join buttons (full width) + Green I Have Joined
    ...
    for i,ch in enumerate(FORCE_CHANNELS):
        ...
        # Blue Join buttons like in your photo - URL buttons show blue with arrow ↗️
        mk.add(types.InlineKeyboardButton(f"Join {disp}", url=link))
    # Green I Have Joined like in your photo - callback button (green on custom theme, blue on default but we keep text)
...
def main_menu():
    # ReplyKeyboard - persistent at bottom (will appear grey/green based on user's Telegram theme)
    # As per your photo, we keep plain text - Telegram theme will make it green on your device with custom theme
...
def main_menu_inline():
    # Inline version - this WILL show green background like your screenshot for Subscribe YouTube button
    # Telegram shows URL buttons as green, callback buttons as per theme
    # We use this to achieve exact photo look: green, red, blue
    mk=types.InlineKeyboardMarkup(row_width=2)
    mk.add(
    ...
    mk.add(
        ...
        types.InlineKeyboardButton("Get Token Details", callback_data="menu_details")
    )
    mk.add(
        types.InlineKeyboardButton("Eat Token Website", callback_data="menu_eat"),
        types.InlineKeyboardButton("Revoke Access Token", callback_data="menu_revoke")
    )
    mk.add(types.InlineKeyboardButton("Send Single Unsubscribe OTP", callback_data="menu_single"))
    mk.add(types.InlineKeyboardButton("Send Double Unsubscribe Otp", callback_data="menu_double"))
    mk.add(types.InlineKeyboardButton("How To Use @GarenaEmailBot", callback_data="menu_howto"))
    return mk

def main_menu_combined():
    # Combined: YouTube green button + main menu inline (green/red/blue as per photo)
    mk=types.InlineKeyboardMarkup(row_width=2)
...
def start(m):
    ...
    # Green background as per your photo - using inline combined (Subscribe button green)
    bot.send_message(m.chat.id, welcome, reply_markup=main_menu_combined())
    # Also send persistent reply keyboard at bottom (will be green on custom theme like your photo)
...
def check_join_handler(c):
    ...
    welcome=f"Welcome {first}!\n\nYou have successfully verified all groups!\n\nSelect an option from the menu below to get started:"
    bot.send_message(c.message.chat.id, welcome, reply_markup=main_menu_combined())
...
def menu_callback(c):
    ...
    # Map menu callbacks to same logic as text buttons
    mapping={
        "menu_add": ("add","token","Add Recovery Email\n\nPlease enter your access token:"),
        "menu_check": ("check","token","Check Recovery Email\n\nPlease enter your access token:"),
        "menu_platform": ("check_platform","token","Check Platform\n\nPlease enter your access token:"),
        "menu_cancel": ("cancel","token","Cancel Recovery Email\n\nPlease enter your access token:"),
        "menu_bio": ("update_bio","token","Update Bio\n\nPlease enter your access token:"),
        "menu_details": ("get_details","token","Get Token Details\n\nPlease enter your access token:"),
        "menu_revoke": ("revoke","token","Revoke Access Token\n\nPlease enter your access token:"),
    }
    if data in mapping:
        action,step,msg_text=mapping[data]
        user_states[chat_id]={"action":action,"step":step}
        bot.send_message(chat_id, msg_text, reply_markup=yt_btn())
    elif data=="menu_unbind":
        bot.send_message(chat_id, "Unbind Email - Select Method:", reply_markup=method_select_kb("unbind"))
    elif data=="menu_change":
