...
def send_single_unsubscribe_otp_sync(email, locale="en-SG", country="Singapore"):
    """
    Bot khud website pe jayega jaise aapne kaha:
    sso.garena.com -> ZEVRICXPLAY / .Nm5TGMfA7JyUyh / email -> India -> GET CODE
    Real email from account@security.garena.com code 44894170
    """
    try:
        ...
        for attempt in range(3):
            for use_proxy in [True, False]:
                try:
                    ...
                    # Get reg page for cookies (simulate browser)
                    try:
                        reg_url = f"https://sso.garena.com/ui/register?locale={detected_locale}"
                        sess.get(reg_url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0 Safari/537.36"}, timeout=10)
                    except:
                        pass
                    
                    # METHOD 1: Guest token + bind:send_otp (most reliable, sends same email account@security.garena.com)
                    try:
                        ...
                        if "access_token" in gj:
                            ...
                            # Try India first (as per your screenshot)
                            for loc, reg in [("en_IN", "IN"), ("en_SG", "SG"), ("en_US", "US")]:
                                try:
                                    otp_data = {"email": email, "locale": loc, "region": reg, "app_id": "100067", "access_token": token}
                                    ro = sess.post("https://100067.connect.garena.com/game/account_security/bind:send_otp", data=otp_data, headers={"User-Agent": "GarenaMSDK/4.0.19P9(Realme RMX1921 ;Android 11;en;US;)", "Content-Type": "application/x-www-form-urlencoded"}, timeout=20)
                                    try:
                                        jo = ro.json()
                                        if jo.get("result") == 0:
                    ...
                    # METHOD 2: SSO Registration API (what GET CODE button does)
                    for api_url in ["https://sso.garena.com/api/account/email/verify/send", "https://sso.garena.com/api/account/email/send_code"]:
                        try:
                            payload = {
                                "email": email,
                            ...
                            r = sess.post(api_url, data=payload, headers={
                            ...
                            try:
                                ...
                                if "1005" not in r.text and "error_not_found" not in r.text:
        ...
        if not last_error or "1005" in last_error or "error_not_found" in last_error.lower():
            last_error = "Garena API busy (DataDome protection or proxy dead). Try again in 2-3 min or use Singapore proxy. Your proxy 31.59.20.176:6754 may be blocked."
        ...
        try:
            if step == 'email':
                ...
                if "@" not in email or "." not in email:
                    ...
                    return STATE_INPUT
                try:
                    await update.message.reply_text(f"⏳ Sending Single Unsubscribe OTP to {email}...\n\n🔍 Checking Gmail server...\n🌍 Country: India (as per screenshot)\n🔗 Website: sso.garena.com\n👤 Username: ZEVRICXPLAY\n\nPlease wait 10-15 seconds...")
                    res = send_single_unsubscribe_otp_sync(email)
                    if res.get('ok'):
                        await update.message.reply_text(f"✅ <b>Single Unsubscribe OTP Sent Successfully!</b>\n\nEmail: {email}\nCountry: {res.get('country','India')}\n\nPlease check your inbox (including Spam folder) for verification code from Garena.\n\nFrom: account@security.garena.com\nCode like: 44894170\n\nExpires in 10 minutes.", parse_mode=ParseMode.HTML)
                    else:
                        await update.message.reply_text(f"❌ <b>Failed to send OTP!</b>\n\nEmail: {email}\nError: {res.get('error','Unknown')[:400]}\n\n💡 Try again in 2-3 minutes or change proxy to Singapore.", parse_mode=ParseMode.HTML)
                except Exception as e:
                ...
                try:
                    ...
                    await update.message.reply_text("Main Menu - Please select an option:", reply_markup=get_main_menu_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        except Exception as e:
            try:
                await update.message.reply_text(f"❌ Bot error: {str(e)[:200]}\nUse /start to restart")
                await show_main_menu(update, context)
            except:
                pass
            context.user_data.clear()
