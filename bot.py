...
def main_menu():
    # TRUE FULL COLOR BOTTOM KEYBOARD - moves green options to where grey keyboard was
    # User wants options at bottom (ReplyKeyboard position) with full green/red/blue
    # Using raw dict with style field for Bot API 9.x
    keyboard = [
        ...
        [{"text": "Send Single Unsubscribe OTP", "style": "success"}, {"text": "Send Double Unsubscribe OTP", "style": "success"}],
        [{"text": "How To Use @GarenaEmailBot", "style": "primary"}]
    ]
    # Return dict - telebot will json dumps it and Telegram will show true green/red/blue bottom keyboard
    return {"keyboard": keyboard, "resize_keyboard": True, "is_persistent": True}

def main_menu_old_grey():
    # Old grey version kept for fallback
...
def main_menu_old():
    # Old bottom menu kept for reference but not used
    m = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    m.add(types.KeyboardButton("🟩 Add Recovery Email"), types.KeyboardButton("🟩 Check Recovery Email"))
    m.add(types.KeyboardButton("🟩 Check Platform"), types.KeyboardButton("🟩 Cancel Recovery Email"))
    m.add(types.KeyboardButton("🟩 Unbind Email"), types.KeyboardButton("🟩 Change Bind Email"))
...
def start(m):
    ...
    # Only bottom full color menu - where grey keyboard was
    bot.send_message(m.chat.id, "Main Menu - Niche se select karo: 👇", reply_markup=main_menu())
...
def all_handler(m):
    ...
    if action == "add_email":
        ...
        if state["step"]=="email":
            ...
            # Exact format like @GarenaEmailBot original screenshot - ALWAYS show success like original
            # Original bot shows success even for existing email yji43043@gmail.com and actually sends code 05530666
