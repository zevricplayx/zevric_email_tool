...
def main_menu():
    # FULL COLOR INLINE MENU - 11 options as per user (removed Change Name, Access To Login, Get Eat Token)
    # Single & Double separate full width
...
def bottom_keyboard():
    # Small persistent bottom keyboard like in your 2nd photo - Eat-Token
    m = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=1)
    m.add(types.KeyboardButton("Eat-Token"))
    return m
...
def start(m):
    ...
    if not all_joined:
        ...
        for ch in FORCE_CHANNELS:
            clean = ch.replace('@','')
            if clean == 'zevricxplay': name="Xirus Apis" if "xirus" in clean.lower() else "Zevricxplay"
            elif 'illigal' in clean: name="Zevric Illigal Vounch"
            elif 'baner' in clean: name="Zevric Baner"
            elif 'all_update' in clean: name="Zevric All Update"
            elif 'api_tools' in clean: name="Zevric Api Tools"
            else: name=clean.replace('_',' ').title()
            # Keep original names for verification
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("menu_") or c.data == "check_join")
def inline_menu_handler(c):
    ...
    mapping = {
        "menu_add": "add_email",
        "menu_check": "check_email",
        "menu_platform": "check_platform",
        "menu_cancel": "cancel",
        "menu_unbind": "unbind",
        "menu_change": "change",
        "menu_bio": "update_bio",
        "menu_name": "change_name",
        "menu_access": "access_login",
        "menu_revoke": "revoke",
        "menu_geteat": "get_eat",
        "menu_eataccess": "eat_access",
        "menu_single": "single",
        "menu_double": "double"
    }
...
# Keep old text handlers for compatibility
@bot.message_handler(func=lambda m: m.text and "Add Recovery Email" in m.text)
def add_btn(m):
    user_states[m.chat.id] = {"action":"add_email","step":"email"}
    bot.send_message(m.chat.id, "Add Recovery Email\n\n📧 Please enter your email address: 👇", reply_markup=yt_btn())

@bot.message_handler(func=lambda m: m.text and "Check Recovery Email" in m.text)
def check_btn(m):
...
@bot.message_handler(func=lambda m: m.text and "Check Platform" in m.text)
def plat_btn(m):
    token = user_tokens.get(m.chat.id)
    if not token:
...
@bot.message_handler(func=lambda m: m.text and "Revoke Access Token" in m.text)
def revoke_btn(m):
    user_states[m.chat.id] = {"action":"revoke","step":"token"}
    bot.send_message(m.chat.id, "Revoke Access Token\n\n🔑 Please enter your access token: 👇", reply_markup=yt_btn())

@bot.message_handler(func=lambda m: m.text and "Get Eat Token" in m.text)
def geteat_btn(m):
    bot.send_message(m.chat.id, "Get Eat Token\n\n🌐 Click below to get Eat Token", reply_markup=eat_btn())
    bot.send_message(m.chat.id, "Main Menu:", reply_markup=main_menu())

@bot.message_handler(func=lambda m: m.text and "Eat To Access" in m.text)
def eataccess_btn(m):
    user_states[m.chat.id] = {"action":"eat_access","step":"token"}
    bot.send_message(m.chat.id, "Eat To Access\n\n🔑 Please enter your Eat token: 👇", reply_markup=yt_btn())

@bot.message_handler(func=lambda m: m.text and "Send Single Unsubscribe" in m.text)
def single_btn(m):
    user_states[m.chat.id] = {"action":"single","step":"email"}
    bot.send_message(m.chat.id, "Send Single Unsubscribe Otp\n\nPlease enter your email address:", reply_markup=yt_btn())

@bot.message_handler(func=lambda m: m.text and "Send Double Unsubscribe" in m.text)
def double_btn(m):
