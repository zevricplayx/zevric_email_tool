...
def main_menu():
    # Bottom ReplyKeyboard - 13 options EXACT like photo - where grey keyboard was
    # Layout as per user's image: 6 rows x 2 + 1 full width = 13
    ...
    try:
        ...
        m.add(types.KeyboardButton("🟩 Send Single Unsubscribe Otp"))
    return m

def main_menu_inline_full_color():
    # Top Inline - EXACT like photo user sent - Full Green + Red
...
def start(m):
    ...
    bot.send_message(m.chat.id, "Full Color Menu:", reply_markup=main_menu_inline_full_color())
