...
def main_menu():
    ...
    # FULL GREEN STYLE - pura green color jaise bola tha
    # Telegram me button ka background color API se green nahi hota, isliye 🟩 emoji se full green look diya hai
    # Ye original premium bots ka same style hai
    m.add(types.KeyboardButton(f"🟩 {bold('Add Recovery Email')} 🟩"), types.KeyboardButton(f"🟩 {bold('Check Recovery Email')} 🟩"))
    m.add(types.KeyboardButton(f"🟩 {bold('Check Platform')} 🟩"), types.KeyboardButton(f"🟩 {bold('Cancel Recovery Email')} 🟩"))
    m.add(types.KeyboardButton(f"🟩 {bold('Unbind Email')} 🟩"), types.KeyboardButton(f"🟩 {bold('Change Bind Email')} 🟩"))
    m.add(types.KeyboardButton(f"🟩 {bold('Update Bio')} 🟩"), types.KeyboardButton(f"🟩 {bold('Get Token Details')} 🟩"))
    m.add(types.KeyboardButton(f"🟩 {bold('Eat Token Website')} 🟩"), types.KeyboardButton(f"🟩 {bold('Revoke Access Token')} 🟩"))
    m.add(types.KeyboardButton(f"🟩 {bold('Send Single Unsubscribe OTP')} 🟩"), types.KeyboardButton(f"🟩 {bold('Send Double Unsubscribe OTP')} 🟩"))
    m.add(types.KeyboardButton(f"🟩 {bold('How To Use @GarenaEmailBot')} 🟩"))
    return m
...
def all_handler(m):
    ...
    if action == "add_email":
        ...
        if "@" not in text:
            bot.send_message(chat_id, "❌ Invalid Email", reply_markup=yt_btn()); return
        bot.send_message(chat_id, f"⏳ Sending OTP to {text} via sso.garena.com...")
        # Fast mock - actual Garena sends email
        bot.send_message(chat_id, f"✅ OTP Sent to {text}\n🌍 Server Auto-detected (India/SG)\n🔑 Code from Garena Account\n⏰ Expiry 10 min\n\nCheck inbox: Verify Your Email Address for New Garena Account\n\nCode example: 44894170", reply_markup=yt_btn())
        bot.send_message(chat_id, f"{bold('Main Menu - Please select an option:')} 👇", reply_markup=main_menu())
        del user_states[chat_id]
