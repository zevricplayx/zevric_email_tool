...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if text_lower == "owner details":
            await update.message.reply_text("👑 Owner: Zevric X Play\n📩 Telegram: @just_zevric\n📺 YouTube: @zevricxplay\n✅ Version: FINAL v6.0 - All Fixed with RAO API", reply_markup=get_youtube_keyboard())
            await update.message.reply_text("📋 Main Menu:", reply_markup=get_reply_keyboard())
            return STATE_INPUT
        ...
        if flow == "bind_info":
            if step == "token":
                ...
                if not bind["ok"]:
                    await update.message.reply_text(f"❌ Error: {bind.get('error')}\nRaw: {bind.get('raw','')[:200]}", reply_markup=get_youtube_keyboard())
                else:
                    data = bind["data"]
        ...
        if flow == "bind":
            ...
            if step == "email":
                if "@" not in text or "." not in text:
                    await update.message.reply_text("❌ Invalid Email! Enter again:", reply_markup=get_youtube_keyboard())
                    return STATE_INPUT
            ...
            if step == "otp":
                ...
                if res["ok"]:
                    ...
                    if vt:
                        ...
                        await update.message.reply_text("❌ Verifier token not found - OTP invalid", reply_markup=get_youtube_keyboard())
                        await update.message.reply_text("📋 Main Menu:", reply_markup=get_reply_keyboard())
                        context.user_data.clear()
                else:
                    await update.message.reply_text(f"❌ OTP Verification Failed: {res.get('data')}", reply_markup=get_youtube_keyboard())
                    await update.message.reply_text("📋 Main Menu:", reply_markup=get_reply_keyboard())
                    context.user_data.clear()
                return STATE_INPUT
            if step == "sec_code":
                if not text.isdigit() or len(text) != 6:
                    await update.message.reply_text("❌ Must be 6 digits! Enter again (e.g. 123456):", reply_markup=get_youtube_keyboard())
                    return STATE_INPUT
        ...
        if flow == "unbind":
            if step == "token":
                ...
                if res["ok"]:
                    ...
                    await update.message.reply_text(f"❌ Failed to send OTP: {res.get('error')}", reply_markup=get_youtube_keyboard())
                    await update.message.reply_text("📋 Main Menu:", reply_markup=get_reply_keyboard())
                    context.user_data.clear()
                return STATE_INPUT
            if step == "otp":
                ...
                if res["ok"]:
                    ...
                    if it:
                        ...
                        if unbind_res["ok"]:
                            await update.message.reply_text("✅ Unbind Request Created! Check email for confirmation.", reply_markup=get_youtube_keyboard())
                        else:
                            await update.message.reply_text(f"❌ Unbind failed: {unbind_res['data']}", reply_markup=get_youtube_keyboard())
                    else:
                        await update.message.reply_text("❌ Identity token missing", reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(f"❌ OTP Failed: {res.get('data')}", reply_markup=get_youtube_keyboard())
                await update.message.reply_text("📋 Main Menu:", reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "change":
            if step == "token":
                ...
                if not old_email:
                    await update.message.reply_text("❌ No Bound Email Found. Use BIND EMAIL first.", reply_markup=get_youtube_keyboard())
                    await update.message.reply_text("📋 Main Menu:", reply_markup=get_reply_keyboard())
                    context.user_data.clear()
                    return STATE_INPUT
                context.user_data["old_email"] = old_email
                await update.message.reply_text(f"📧 Current: {old_email}\n⏳ Sending OTP to old email...", reply_markup=get_youtube_keyboard())
                res = send_otp_sync(old_email, text)
                if res["ok"]:
                    await update.message.reply_text(f"✅ OTP Sent to {old_email}\n📩 Enter OTP:", reply_markup=get_youtube_keyboard())
            ...
            if step == "old_otp":
                ...
                if res["ok"]:
                    ...
                    await update.message.reply_text("✅ Old Email Verified!\n📧 Enter NEW Email:", reply_markup=get_youtube_keyboard())
                    context.user_data["step"] = "new_email"
                else:
                    await update.message.reply_text(f"❌ Verification Failed: {res.get('data')}", reply_markup=get_youtube_keyboard())
                    await update.message.reply_text("📋 Main Menu:", reply_markup=get_reply_keyboard())
                    context.user_data.clear()
                return STATE_INPUT
            if step == "new_email":
            ...
            if step == "new_otp":
                ...
                if res["ok"]:
                    ...
                    if vt and context.user_data.get("identity_token"):
                        ...
                        if rebind["ok"]:
                            await update.message.reply_text(f"✅ Change Request Created!\n{context.user_data['old_email']} -> {context.user_data['new_email']}", reply_markup=get_youtube_keyboard())
                        else:
                            await update.message.reply_text(f"❌ Change failed: {rebind['data']}", reply_markup=get_youtube_keyboard())
                    else:
                        await update.message.reply_text("❌ Token missing", reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(f"❌ New OTP Failed: {res.get('data')}", reply_markup=get_youtube_keyboard())
                await update.message.reply_text("📋 Main Menu:", reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "sec_info":
            if step == "token":
                ...
                if bind["ok"]:
                    ...
                    msg = f"🔒 SECURITY CODE INFO\n\n👤 {nick} ({uid})\n🌍 {region}\n🔒 Security: {sec_status}\n📧 Email: {bind['data'].get('email','None')}"
                else:
                    msg = f"❌ Failed: {bind.get('error')}"
                await update.message.reply_text(msg, reply_markup=get_youtube_keyboard())
                await update.message.reply_text("📋 Main Menu:", reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "change_sec":
            if step == "token":
                ...
                email = bind["data"].get("email", "") if bind["ok"] else ""
                if not email:
                    await update.message.reply_text("❌ No Bound Email. Bind email first.", reply_markup=get_youtube_keyboard())
                    await update.message.reply_text("📋 Main Menu:", reply_markup=get_reply_keyboard())
                    context.user_data.clear()
                    return STATE_INPUT
                context.user_data["email"] = email
            ...
            if step == "new_code":
                if not text.isdigit() or len(text) != 6:
                    await update.message.reply_text("❌ Must be 6 digits! Example 123456:", reply_markup=get_youtube_keyboard())
                    return STATE_INPUT
        ...
        if flow == "fix_unsub":
            if step == "email":
                ...
                try:
                    ...
                    await update.message.reply_text(f"✅ Single Unsubscribe OTP Sent!\n📧 Email: {email}\n📩 Check inbox + Spam folder", reply_markup=get_youtube_keyboard())
                except Exception as e:
                    await update.message.reply_text(f"✅ OTP Sent to {email} (attempted)\nError: {str(e)[:100]}", reply_markup=get_youtube_keyboard())
                await update.message.reply_text("📋 Main Menu:", reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        traceback.print_exc()
        try:
            await update.message.reply_text("❌ Error Occurred. Use /start", reply_markup=get_reply_keyboard())
        except:
            pass
        context.user_data.clear()
        return STATE_INPUT
...
@flask_app.route('/')
def home():
    return "✅ Bot Running - FINAL v6.0 RAO API Fixed - All Options Working"
