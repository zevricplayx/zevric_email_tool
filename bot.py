        ...
        if flow == "single_unsub":
            if step == "email":
                ...
                if res.get("ok"):
                    ...
                    await update.message.reply_text(ffancy(f"Failed To Send OTP: {res.get('error','No response')}"), reply_markup=get_youtube_keyboard())
                await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "bind_info":
            if step == "token":
                ...
                if not bind["ok"]:
                    ...
                    if email_to_be:
                        msg = ffancy(f"Email Information\n\nEmail: {email_to_be}\nConfirm In: {countdown_human}")
                    else:
                        if email:
                            msg = ffancy(f"Email Information\n\nEmail: {email}\nConfirm In: No Pending Request")
                        else:
                            msg = ffancy("Email Information\n\nEmail: None\nConfirm In: No Pending Request")
