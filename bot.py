...
def send_single_unsubscribe_otp_sync(email, locale="en-SG", country="Singapore"):
    """FAST 5-10 SEC - ALWAYS USE PROXY (hardcoded, no env needed) - website needs VPN"""
    try:
        import random
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # ===== BIND INFO - DEEP RESEARCHED: PLAYER + BIND + PENDING + COUNTDOWN =====
        if flow == "bind_info":
            if step == "token":
                ...
                if not bind["ok"]:
                    ...
                    has_secondary = data.get("has_secondary_password", False)
                    request_type = data.get("request_type", 0)
                    
                    msg = "📧 BIND INFO\n\n"
                    if player.get("ok"):
                        msg += f"👤 Player Info:\n• UID: {player.get('uid')}\n• Nickname: {player.get('nickname')}\n• Region: {player.get('region')}\n\n"
                    msg += f"🔐 Bind Details:\n• Current Email: {email if email else 'None'}\n• Pending Email: {email_to_be if email_to_be else 'None'}\n"
                    if email_to_be:
                        msg += f"• Countdown: {countdown_human} ({countdown}s)\n"
                        msg += f"• Request Type: {request_type}\n"
                    msg += f"• Has Secondary Password: {has_secondary}\n"
                    msg += f"• Result: {data.get('result', 'Unknown')}\n"
                    if countdown > 0:
                        msg += f"\n⏳ Pending request will execute in {countdown_human}"
                    else:
                        if not email_to_be:
                            msg += "\n✅ No pending request"
