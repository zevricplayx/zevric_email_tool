...
def send_otp_sync(email, access_token):
    try:
        ...
        if "captcha" in raw_lower or "geo.captcha-delivery.com" in raw_lower:
            return {"ok": False, "data": j, "raw": r.text, "captcha": True}
...
def send_single_unsubscribe_otp_sync(email):
    try:
        ...
        for url in ["https://sso.garena.com/api/account/email/unsubscribe/send_otp"]:
            try:
...
# ===== CLEAN INLINE KEYBOARD - ONLY ONE SET (NO DUPLICATE) =====
def get_main_menu_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("Send Double Unsubscribe Otp", callback_data="main_double_unsub")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # REMOVE old ReplyKeyboard if any (fixes upar-niche duplicate issue)
    try:
        await update.message.reply_text("Removing old keyboard...", reply_markup=ReplyKeyboardRemove())
    except: pass
    welcome = "⚡ <b>Garena Email Bot</b> ⚡\n\n👨‍💻 Developer: @raostarr\n📺 YouTube: @raostarrr\n🔐 Status: SAFE & SECURE\n\nSelect an option from the menu below to get started:\n\n<b>Note:</b> Telegram inline buttons me color green/red/blue API se set nahi hota - grey hi dikhega. Green wala screenshot ReplyKeyboard ka hai jo theme par depend karta hai."
...
async def cancel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await show_main_menu(update, context, "Main Menu - Please select an option:"); return STATE_INPUT
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_howto":
        text = "📘 <b>How To Use @GarenaEmailBot</b>\n\n1️⃣ /start se menu kholo\n2️⃣ Option choose karo\n3️⃣ Access Token bhejo\n4️⃣ OTP / Sec Code follow karo\n\n<b>Color Issue:</b> Telegram inline buttons me green/red color API se possible nahi hai. Grey normal hai. Agar green chahiye to ReplyKeyboard use karna padega.\n\n<b>Captcha Error Fix:</b> Agar 'geo.captcha-delivery.com' error aaye to:\n• Via Sec Code try karo\n• Ya Send Single Unsubscribe OTP use karo\n• Ya 10 min baad try karo\n\n📺 YT: @raostarrr"
    ...
    if data == "main_double_unsub":
        context.user_data.clear(); context.user_data['flow'] = 'double_unsub'; context.user_data['step'] = 'email'
        await query.message.reply_text("📧 <b>Send Double Unsubscribe OTP</b>\n\nJis email pe OTP bhejna hai wo bhejo:", parse_mode=ParseMode.HTML); return STATE_INPUT
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if flow == 'bind_email':
        ...
        if step == 'email':
            ...
            if res['ok']: await update.message.reply_text(f"✅ OTP Sent!\n\nEmail pe aaya hua <b>OTP</b> bhejo:", parse_mode=ParseMode.HTML); context.user_data['step'] = 'otp'
                ...
                if res.get('captcha'):
                    await update.message.reply_text("⚠️ <b>Garena Captcha Block!</b>\n\nGarena ne Render IP ko block kiya hai.\n\n<b>Fix:</b>\n• <b>Via Sec Code</b> method use karo (OTP ki jagah)\n• <b>Send Single Unsubscribe OTP</b> wala option try karo\n• 10 min baad dubara try karo\n\nRaw captcha URL hide kar diya hai - ye normal error hai jab Garena datacenter IP detect karta hai.", parse_mode=ParseMode.HTML)
        ...
        if step == 'otp':
            ...
            if res['ok'] and res['data'].get('verifier_token'):
                ...
                if bind_req['ok']: await update.message.reply_text(f"✅ <b>BIND SUCCESS</b>", parse_mode=ParseMode.HTML)
    ...
    if flow == 'cancel_req':
        if step == 'token':
            ...
            if res['ok']: await update.message.reply_text(f"✅ <b>CANCEL SUCCESS</b>", parse_mode=ParseMode.HTML)
    ...
    if flow == 'revoke':
        if step == 'token':
            ...
            if res['ok']: await update.message.reply_text(f"✅ <b>REVOKE SUCCESS</b>", parse_mode=ParseMode.HTML)
        ...
        if step == 'token':
            ...
            if method == 'otp':
                ...
                if res['ok']: await update.message.reply_text(f"✅ OTP sent to {email}\n\nOTP bhejo:", parse_mode=ParseMode.HTML); context.user_data['step'] = 'otp'
                    ...
                    if res.get('captcha'):
                        await update.message.reply_text("⚠️ <b>Captcha Block!</b>\n\n<b>Via Sec Code</b> method use karo - ye captcha bypass karta hai!", parse_mode=ParseMode.HTML)
                        await show_main_menu(update, context); context.user_data.clear()
                    else:
        ...
        if step == 'token':
            ...
            if method == 'otp':
                ...
                if res['ok']: await update.message.reply_text("✅ OTP sent! Old email ka OTP bhejo:", parse_mode=ParseMode.HTML); context.user_data['step'] = 'old_otp'
                    ...
                    if res.get('captcha'):
                        await update.message.reply_text("⚠️ <b>Garena Captcha Block!</b>\n\n<b>Solution:</b>\n• <b>Via Sec Code</b> wala option choose karo\n• Ye captcha bypass karta hai!", parse_mode=ParseMode.HTML)
                        await show_main_menu(update, context); context.user_data.clear()
                    else:
                        await update.message.reply_text(f"❌ OTP fail: {res['data']}")
            else: await update.message.reply_text("🔒 Apna <b>6-digit Security Code</b> bhejo:", parse_mode=ParseMode.HTML); context.user_data['step'] = 'sec_code'
