        ...
        if flow == "fix_unsub":
            if step == "email":
                ...
                if "@" not in email:
                    ...
                    return STATE_INPUT
                await update.message.reply_text(f"Sending Unsubscribe Fix OTP to {email}...", reply_markup=get_youtube_keyboard())
                try:
                    ...
                    r=session.get("https://sso.garena.com/universal/register?locale=en-SG",headers=headers_base,timeout=10)
                    # Try send OTP
                    payload={"email":email,"username":random_user,"password":random_pass,"locale":"en-SG","region":"SG"}
                    api="https://sso.garena.com/api/account/send_verification_code"
                    resp=session.post(api,headers={**headers_base, "Content-Type":"application/json"},json=payload,timeout=10)
                    await update.message.reply_text(f"SSO Response: {resp.text[:400]}", reply_markup=get_youtube_keyboard())
                except Exception as e:
