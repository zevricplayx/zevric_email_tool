...
user_states = {}
user_tokens = {}
verified_users = {}  # user_id -> timestamp of last verification
...
def revoke_token_api(token):
    try:
        ...
        return r.status_code == 200 and "error" not in r.text.lower(), r.text
    except Exception as e:
        return False, str(e)
...
def is_user_joined(user_id):
    ...
    for channel in FORCE_CHANNELS:
        try:
            ...
            # If bot not admin or channel not found, skip to avoid blocking
            print(f"getChatMember error for {channel}: {e}")
            pass
...
def force_join_markup():
    ...
    mk.add(types.InlineKeyboardButton("✅ I Have Joined - Verify", callback_data="check_join"))
    return mk

def main_menu_colored():
    # Removed SSO Garena Register Website as requested
    return {
        "keyboard": [
...
def send_main_menu(chat_id, text="Main Menu - Please select an option:"):
    try:
        ...
        payload = {"chat_id": chat_id, "text": text, "reply_markup": main_menu_colored()}
        r = requests.post(url, json=payload, timeout=10)
        if r.status_code != 200:
            raise Exception(f"API {r.status_code}: {r.text[:100]}")
    except Exception as e:
        print(f"Colored menu failed, fallback: {e}")
...
def check_and_enforce_join(user_id, chat_id):
    """Check if user joined all required channels, if not enforce. Returns True if joined, False if not."""
    all_joined, not_joined = is_user_joined(user_id)
    if not all_joined:
        msg = "⚠️ Join Verification Required\n\n"
        msg += "To use this bot, you must join the following groups/channels first:\n\n"
        for ch in FORCE_CHANNELS:
            ...
            status = "❌" if ch in not_joined else "✅"
            msg+=f"{status} {disp} ({ch})\n"
        msg+="\nAfter joining all, click the button below to verify:\n\n"
        msg+="Note: If you leave any channel/group, you will need to verify again!"
...
def all_handler(m):
    ...
    # PERSISTENT FORCE JOIN CHECK - if user leaves any channel, re-verify every time
    # Skip check for verification callback itself, but check for all other actions
    if text != "/start" and not text.startswith("✅"):
        # Always re-check membership on every action
    ...
    if action=="add_email":
        if step=="email":
            if "@" not in text:
                bot.send_message(chat_id, "Invalid Email", reply_markup=main_menu())
                send_main_menu(chat_id, "Main Menu:")
                return
            ...
            if not is_token(text):
                bot.send_message(chat_id, "Invalid Token!", reply_markup=main_menu())
                send_main_menu(chat_id, "Main Menu:")
                return
        ...
        if step=="email":
            ...
            try:
                ...
                if success:
                    bot.send_message(chat_id, f"SINGLE UNSUBSCRIBE FIXED!\n\nEmail: {email}\nOTP Verified: {otp}\nStatus: Resubscribed to Garena", reply_markup=yt_btn())
                    resubscribe_garena_email(email)
                else:
                    bot.send_message(chat_id, f"OTP Verification Failed: {resp[:600]}\nIf code expired, request new OTP", reply_markup=yt_btn())
            except Exception as e:
                bot.send_message(chat_id, f"Verify Error: {e}", reply_markup=yt_btn())
            del user_states[chat_id]
            send_main_menu(chat_id, "Main Menu:")
        ...
        if step=="email":
            ...
            try:
                success, resp = verify_garena_otp(email, otp)
                if success:
                    bot.send_message(chat_id, f"DOUBLE UNSUBSCRIBE FIXED!\n\nEmail: {email}\nOTP Verified: {otp}\nRemoved from BOTH suppression lists\n\nYou will NOW receive all Garena OTPs again!", reply_markup=yt_btn())
                else:
                    bot.send_message(chat_id, f"OTP Received: {otp}\n\nYour email {email} is being resubscribed. Check Gmail after 5 mins.", reply_markup=yt_btn())
            except Exception as e:
        ...
        if step=="token":
            ...
            if action=="cancel":
                try:
                    ...
                    if r.json().get("result")==0:
                        bot.send_message(chat_id, "Cancel Recovery Email Successful!", reply_markup=yt_btn())
                    else:
                        bot.send_message(chat_id, f"Cancel Failed: {r.text[:500]}", reply_markup=yt_btn())
                except Exception as e:
                    bot.send_message(chat_id, f"Error: {e}", reply_markup=yt_btn())
            elif action=="revoke":
                success, resp = revoke_token_api(text)
                if success:
                    uid,nick,region = get_player_info(text)
                    bot.send_message(chat_id, f"REVOKED\nNickname: {nick}\nAccount ID: {uid}\nRegion: {region}\nStatus: Successfully Logged Out & Revoked", reply_markup=yt_btn())
                else:
                    bot.send_message(chat_id, f"Failed to revoke: {resp[:500]}", reply_markup=yt_btn())
            del user_states[chat_id]
            send_main_menu(chat_id, "Main Menu:")

@app.route('/')
def home(): return "BOT RUNNING - FINAL PERSISTENT JOIN CHECK - NO SSO - ALWAYS COLORED"
@app.route('/health')
