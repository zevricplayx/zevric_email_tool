...
def get_youtube_keyboard():
    try:
        return InlineKeyboardMarkup([
            [InlineKeyboardButton(text="🔴 📺 SUBSCRIBE - Zevric X Play ↗️", url=YOUTUBE_LINK)],
            [InlineKeyboardButton(text="❤️ LIKE | 🔔 BELL | 📢 SHARE", url=YOUTUBE_LINK)]
        ])
    except:
        return InlineKeyboardMarkup([[InlineKeyboardButton(text="🔴 📺 SUBSCRIBE - Zevric X Play ↗️", url=YOUTUBE_LINK)]])
    ...
    try:
        ...
        if "single" in text.lower() and "unsub" in text.lower():
            ...
            await update.message.reply_text(
                ...
                "🤖 **Bot will go to:**\n"
                "`sso.garena.com/universal/register?locale=en-SG`\n"
                "Fill `ZEVRICXPLAY` / `.Nm5TGMfA7JyUyh` / your email\n"
                "Detect India server, select India, and click GET CODE 🇮🇳",
        ...
        if flow == "single_unsub":
            if step == "email":
                ...
                try:
                    await update.message.reply_text(
                        f"⏳ **Sending Single Unsubscribe OTP to**\n`{email}`...\n\n"
                        f"📡 Checking server... 🌍 Country: **India** 🇮🇳\n"
                        f"🌐 Website: `sso.garena.com/universal/register?locale=en-SG`\n"
                        f"👤 Username: `ZEVRICXPLAY`\n"
                        f"🔑 Password: `.Nm5TGMfA7JyUyh`\n"
                        f"📧 Email: `{email}`\n"
                        f"🔘 Clicking **GET CODE**...",
                        reply_markup=get_youtube_keyboard(),
                        parse_mode="Markdown"
                    ...
                    if res.get("ok"):
                        ...
                        await update.message.reply_text(
                            ...
                            f"📧 Email: `{email}`\n"
                            f"❗ Error: `{res.get('error','Unknown')[:350]}`\n\n"
                            f"🔄 **Try again:** Wait 2-3 minutes, check email format, and retry.\n"
                            f"🌐 Garena DataDome may be active - using Singapore proxy.",
                            reply_markup=get_youtube_keyboard(),
                            parse_mode="Markdown"
                        )
                except Exception as e:
