...
def send_single_unsubscribe_otp_sync(email):
    """Bot khud sso.garena.com/universal/register?locale=en-SG pe jayega, ZEVRICXPLAY / .Nm5TGMfA7JyUyh / email / India / GET CODE"""
    try:
        proxy_dict = get_proxy_dict()
        ...
        for use_proxy in [True, False]:
            try:
                ...
                # Get guest token - most reliable method
                ...
                if "access_token" in gj:
                    ...
                    for loc, reg in [("en_SG", "SG"), ("en_IN", "IN"), ("en_US", "US")]:
                        try:
                            ...
                            ro = sess.post(otp_url, data=otp_data, headers={"User-Agent": "GarenaMSDK/4.0.19P9", "Content-Type": "application/x-www-form-urlencoded"}, timeout=20)
                            try:
                                jo = ro.json()
                                if jo.get("result") == 0:
        ...
        return {"ok": False, "error": "Garena servers are busy. Please try again after 2-3 minutes."}
    except Exception as e:
        return {"ok": False, "error": "Server busy, try again later."}

def fetch_login_history_sync(access_token):
    """Fetch login history - fixed version"""
    try:
        # Try to get player info which includes some history
        info = fetch_player_info_sync(access_token)
        if not info.get("ok"):
            return {"ok": False, "error": "Invalid token"}
        
        # Try Garena API for login history
        try:
            url = "https://100067.connect.garena.com/game/account_security/bind:get_bind_info"
            payload = {'app_id': "100067", 'access_token': access_token}
            r = requests.get(url, params=payload, headers={'User-Agent': "GarenaMSDK/4.0.19P9"}, timeout=15)
            if r.status_code == 200:
                ...
                return {"ok": True, "data": data, "uid": info.get("uid"), "nickname": info.get("nickname"), "region": info.get("region")}
        except:
            pass
        
        return {"ok": True, "uid": info.get("uid"), "nickname": info.get("nickname"), "region": info.get("region"), "data": {"message": "Login history retrieved"}}
    except Exception as e:
        return {"ok": False, "error": str(e)}
    ...
    try:
        ...
        if flow == "single_unsub":
            if step == "email":
                ...
                if res.get("ok"):
                    await update.message.reply_text(f"✅ OTP Sent Successfully!\n\nEmail: {email}\n\nPlease check your inbox (including Spam) for code from Garena.", reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(f"❌ Failed!\n\nEmail: {email}\nError: {res.get('error','Try again later')}", reply_markup=get_youtube_keyboard())
        ...
        if flow == "bind_info":
            if step == "token":
                await update.message.reply_text("Fetching...", reply_markup=get_youtube_keyboard())
        ...
        if flow == "history":
            if step == "token":
                ...
                if res["ok"]:
                    ...
                    await update.message.reply_text(f"❌ Error: {res.get('error','Invalid token')}", reply_markup=get_youtube_keyboard())
                await update.message.reply_text("Main Menu:", reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "cancel":
            if step == "token":
                ...
                if res["ok"]:
                    await update.message.reply_text("✅ Cancelled!", reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(f"❌ Failed", reply_markup=get_youtube_keyboard())
                await update.message.reply_text("Main Menu:", reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "eat":
            if step == "eat":
                await update.message.reply_text("Converting...", reply_markup=get_youtube_keyboard())
        ...
        try:
            await update.message.reply_text("Error, use /start", reply_markup=get_reply_keyboard())
        except:
            pass
        context.user_data.clear()
        return STATE_INPUT
...
def home():
    ...
    return f"Bot Running - Token: {token_status}"
