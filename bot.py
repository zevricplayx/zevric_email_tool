...
def fancy(t):
    ...
    for ch in t:
        if 'A' <= ch <= 'Z':
            r+=chr(ord(ch)-65+0x1D400)
        elif 'a' <= ch <= 'z':
            r+=chr(ord(ch)-97+0x1D41A)
        elif '0' <= ch <= '9':
            r+=chr(ord(ch)-48+0x1D7CE)
        else:
            r+=ch
    return r

def defancy(t):
    # Convert fancy bold back to normal ascii for logic checks
    r=""
    for ch in t:
        code = ord(ch)
        if 0x1D400 <= code <= 0x1D419:
            r+=chr(code-0x1D400+65)
        elif 0x1D41A <= code <= 0x1D433:
            r+=chr(code-0x1D41A+97)
        elif 0x1D7CE <= code <= 0x1D7D7:
            r+=chr(code-0x1D7CE+48)
        else:
            r+=ch
    return r
...
def fetch_bind_info_sync(access_token):
    try:
        ...
        sess.verify = False
        sess.proxies.update(get_proxy_dict())
        r = sess.get(url, params=payload, headers=headers, timeout=12)
...
def send_otp_sync(email, access_token):
    try:
        ...
        sess.verify = False
        sess.proxies.update(get_proxy_dict())
        r = sess.post(url, headers=headers, data=data, timeout=12)
        j = r.json()
        ...
        for proxy_dict in proxy_options:
            for attempt in range(3):
                try:
                    ...
                    try:
                        gr = sess.post(g_url, data=g_data, headers=guest_headers, timeout=12)
                        gj = gr.json()
                    except Exception as e1:
                        last_error = str(e1)[:150]
                        try:
                            gr = sess.get(f"{g_url}?app_id=100067&udid={udid}&client_id=100067&client_secret=8ba7d83f1f3a0d1d5a0a5b7a1b5e7a0e5", headers=guest_headers, timeout=12)
                            gj = gr.json()
                        except Exception as e2:
                            last_error = str(e2)[:150]
                            continue
                    if "access_token" not in gj:
                        last_error = str(gj)[:150]
                        continue
                    ...
                    for loc, reg in [("en_SG", "SG"), ("en_IN", "IN")]:
                        try:
                            ...
                            ro = sess.post(otp_url, data=otp_data, headers=guest_headers, timeout=12)
                            jo = ro.json()
                            if jo.get("result") == 0 or '"result":0' in ro.text:
                                return {"ok": True, "data": jo, "email": email}
                            else:
                                last_error = str(jo)[:150]
                        except Exception as e3:
                            last_error = str(e3)[:150]
                            continue
                except Exception as e4:
                    last_error = str(e4)[:150]
                    continue
        return {"ok": False, "error": last_error, "email": email}
    except Exception as e:
        return {"ok": False, "error": str(e)[:150], "email": email}
...
def eat_to_token_sync(eat_input):
    try:
        ...
        r = requests.get(api_url, headers={"User-Agent": "Mozilla/5.0"}, allow_redirects=True, timeout=12)
...
def revoke_token_sync(access_token):
    try:
        ...
        logout_res = requests.get(logout_url, headers={"User-Agent": "Mozilla/5.0"}, timeout=12)
...
def fetch_login_history_sync(token_input):
    ...
    try:
        ...
        if token_input.startswith("ey") and "." in token_input:
            ...
            oId = None
            try:
                r = requests.get(f"https://100067.connect.garena.com/oauth/token/inspect?token={token_input}", headers={"User-Agent": "Mozilla/5.0"}, timeout=8).json()
                oId = r.get("open_id")
            except:
                pass
            if not oId:
...
def get_reply_keyboard():
    if HAS_STYLE:
        keyboard = [
            [KeyboardButton(text=fancy("Check Recovery Email"), style=KeyboardButtonStyle.SUCCESS), KeyboardButton(text=fancy("Bind Email"), style=KeyboardButtonStyle.SUCCESS)],
            [KeyboardButton(text=fancy("Unbind Email"), style=KeyboardButtonStyle.SUCCESS), KeyboardButton(text=fancy("Change Bind Email"), style=KeyboardButtonStyle.SUCCESS)],
            [KeyboardButton(text=fancy("Cancel Bind Request"), style=KeyboardButtonStyle.SUCCESS), KeyboardButton(text=fancy("Eat To Access Token"), style=KeyboardButtonStyle.SUCCESS)],
            [KeyboardButton(text=fancy("Revoke Access Token"), style=KeyboardButtonStyle.DANGER), KeyboardButton(text=fancy("Get Login History"), style=KeyboardButtonStyle.SUCCESS)],
            [KeyboardButton(text=fancy("Check Bound Accounts"), style=KeyboardButtonStyle.SUCCESS), KeyboardButton(text=fancy("Send Single Unsubscribe Otp"), style=KeyboardButtonStyle.SUCCESS)],
            [KeyboardButton(text=fancy("Send Double Unsubscribe Otp"), style=KeyboardButtonStyle.SUCCESS), KeyboardButton(text=fancy("Owner Details"), style=KeyboardButtonStyle.PRIMARY)],
        ]
    else:
        keyboard = [
            [fancy("Check Recovery Email"), fancy("Bind Email")],
            [fancy("Unbind Email"), fancy("Change Bind Email")],
            [fancy("Cancel Bind Request"), fancy("Eat To Access Token")],
            [fancy("Revoke Access Token"), fancy("Get Login History")],
            [fancy("Check Bound Accounts"), fancy("Send Single Unsubscribe Otp")],
            [fancy("Send Double Unsubscribe Otp"), fancy("Owner Details")],
        ]
...
def get_method_keyboard():
    if HAS_STYLE:
        keyboard = [
            [KeyboardButton(text=fancy("Via Email Otp"), style=KeyboardButtonStyle.SUCCESS), KeyboardButton(text=fancy("Via Security Code"), style=KeyboardButtonStyle.SUCCESS)],
            [KeyboardButton(text=fancy("Back To Menu"), style=KeyboardButtonStyle.DANGER)],
        ]
    else:
        keyboard = [[fancy("Via Email Otp"), fancy("Via Security Code")], [fancy("Back To Menu")]]
    return ReplyKeyboardMarkup(keyboard, resize_keyboard=True, is_persistent=True)

def get_youtube_keyboard():
    try:
        if HAS_STYLE:
            return InlineKeyboardMarkup([[InlineKeyboardButton(text=fancy("Subscribe YouTube Channel"), url=YOUTUBE_LINK, style=KeyboardButtonStyle.SUCCESS)]])
        else:
            return InlineKeyboardMarkup([[InlineKeyboardButton(text=fancy("Subscribe YouTube Channel"), url=YOUTUBE_LINK)]])
    except:
        return InlineKeyboardMarkup([[InlineKeyboardButton(text=fancy("Subscribe YouTube Channel"), url=YOUTUBE_LINK)]])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = fancy(f"Welcome {first_name}!\n\nSelect An Option From Below Menu")
    await update.message.reply_text(welcome, reply_markup=get_youtube_keyboard())
    await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
    return STATE_INPUT
...
async def cancel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data.clear()
    await update.message.reply_text(fancy("Cancelled. Main Menu"), reply_markup=get_reply_keyboard())
    return STATE_INPUT
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        text_normal = defancy(raw_text).lower()
        text = raw_text.strip()
        flow = context.user_data.get("flow")
        step = context.user_data.get("step")

        # Global keyword detection for all 12 options - works with fancy and normal
        if "double" in text_normal and "unsub" in text_normal:
            await update.message.reply_text(fancy("Double Unsubscribe Otp\n\nComing Soon..."), reply_markup=get_reply_keyboard())
            return STATE_INPUT

        if "single" in text_normal and "unsub" in text_normal:
            context.user_data.clear()
            context.user_data["flow"] = "single_unsub"
            context.user_data["step"] = "email"
            await update.message.reply_text(fancy("Please Enter Your Email Address:"), reply_markup=get_youtube_keyboard())
            return STATE_INPUT

        if "check recovery" in text_normal or "check bind" in text_normal or "recovery email" in text_normal:
            context.user_data.clear()
            context.user_data["flow"] = "bind_info"
            context.user_data["step"] = "token"
            await update.message.reply_text(fancy("Please Enter Your Access Token:"), reply_markup=get_youtube_keyboard())
            return STATE_INPUT

        if text_normal == "bind email":
            context.user_data.clear()
        ...
        if text_normal == "unbind email":
            context.user_data.clear()
            context.user_data["flow"] = "unbind"
            context.user_data["step"] = "method"
            await update.message.reply_text(fancy("Choose Unbind Method:"), reply_markup=get_method_keyboard())
            return STATE_INPUT

        if text_normal == "change bind email":
            context.user_data.clear()
            context.user_data["flow"] = "change"
            context.user_data["step"] = "method"
            await update.message.reply_text(fancy("Choose Change Method:"), reply_markup=get_method_keyboard())
            return STATE_INPUT

        if text_normal == "cancel bind request":
            context.user_data.clear()
        ...
        if "eat" in text_normal and "access" in text_normal:
            context.user_data.clear()
            context.user_data["flow"] = "eat"
            context.user_data["step"] = "eat"
            await update.message.reply_text(fancy("Please Enter EAT Token Or EAT Link:"), reply_markup=get_youtube_keyboard())
            return STATE_INPUT

        if "revoke" in text_normal:
            context.user_data.clear()
        ...
        if "login history" in text_normal:
            context.user_data.clear()
        ...
        if "bound accounts" in text_normal or "check bound" in text_normal:
            context.user_data.clear()
        ...
        if "owner" in text_normal:
            await update.message.reply_text(fancy("Owner: @just_zevric\nYouTube: Zevric X Play\nLink: https://youtube.com/@zevricxplay"), reply_markup=get_youtube_keyboard())
            await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
            return STATE_INPUT

        if "back to menu" in text_normal or text_normal == "back":
            context.user_data.clear()
            await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
            return STATE_INPUT
        ...
        if flow == "single_unsub":
            if step == "email":
                ...
                if "@" not in email or "." not in email:
                    await update.message.reply_text(fancy("Invalid Email! Please Enter Valid Email:"))
                    return STATE_INPUT
                await update.message.reply_text(fancy(f"Sending Single Unsubscribe Otp To {email}..."), reply_markup=get_youtube_keyboard())
                res = send_single_unsubscribe_otp_sync(email)
                if res.get("ok"):
                    await update.message.reply_text(fancy(f"Otp Sent Successfully!\n\nEmail: {email}\n\nPlease Check Your Inbox For Verification Code."), reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(fancy(f"Failed To Send Otp: {res.get('error','No response')}"), reply_markup=get_youtube_keyboard())
                await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "bind_info":
            if step == "token":
                await update.message.reply_text(fancy("Fetching Bind Info..."), reply_markup=get_youtube_keyboard())
                bind = fetch_bind_info_sync(text)
                if not bind["ok"]:
                    await update.message.reply_text(fancy(f"Error: {bind['error']}"), reply_markup=get_youtube_keyboard())
                else:
                    data = bind["data"]
                    ...
                    if email_to_be:
                        ...
                        if email:
                            ...
                            msg = f"Email Information\n\nEmail: None\nConfirm In: No Pending Request"
                    await update.message.reply_text(fancy(msg), reply_markup=get_youtube_keyboard())
                await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "bind":
            if step == "token":
                ...
                await update.message.reply_text(fancy("Please Enter New Email To Bind:"), reply_markup=get_youtube_keyboard())
                context.user_data["step"] = "email"
                return STATE_INPUT
            if step == "email":
                context.user_data["email"] = text
                await update.message.reply_text(fancy(f"Sending Otp To {text}..."), reply_markup=get_youtube_keyboard())
                res = send_otp_sync(text, context.user_data["token"])
                if res["ok"]:
                    await update.message.reply_text(fancy(f"Otp Sent To {text}, Please Enter Otp:"), reply_markup=get_youtube_keyboard())
                    context.user_data["step"] = "otp"
                else:
                    await update.message.reply_text(fancy(f"Failed To Send Otp: {res.get('data')}"), reply_markup=get_youtube_keyboard())
                return STATE_INPUT
            if step == "otp":
                res = verify_otp_sync(context.user_data["email"], context.user_data["token"], text)
                if res["ok"]:
                    vt = res["data"].get("verifier_token")
                    await update.message.reply_text(fancy("Please Enter Secondary Password (6-Digit):"), reply_markup=get_youtube_keyboard())
                    context.user_data["verifier_token"] = vt
                    context.user_data["step"] = "sec"
                else:
                    await update.message.reply_text(fancy(f"Verify Failed: {res['data']}"))
                return STATE_INPUT
            if step == "sec":
                res = create_bind_request_sync(context.user_data["email"], context.user_data["token"], context.user_data["verifier_token"], text)
                if res["ok"]:
                    await update.message.reply_text(fancy(f"Bind Success: {res['data']}"), reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(fancy(f"Bind Failed: {res['data']}"), reply_markup=get_youtube_keyboard())
                await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "unbind":
            if step == "method":
                if "otp" in text_normal or "email" in text_normal:
                    context.user_data["method"] = "otp"
                else:
                    context.user_data["method"] = "sec"
                await update.message.reply_text(fancy("Please Enter Access Token For Unbind:"), reply_markup=get_youtube_keyboard())
                context.user_data["step"] = "token"
                return STATE_INPUT
            if step == "token":
                context.user_data["token"] = text
                ...
                if not bind["ok"] or not bind["data"].get("email"):
                    await update.message.reply_text(fancy("No Bound Email Found."), reply_markup=get_youtube_keyboard())
                    await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
                    context.user_data.clear()
                    return STATE_INPUT
                ...
                if context.user_data.get("method") == "otp":
                    await update.message.reply_text(fancy(f"Current Email: {email}\nSending Otp..."), reply_markup=get_youtube_keyboard())
                    res = send_otp_sync(email, text)
                    if res["ok"]:
                        await update.message.reply_text(fancy(f"Otp Sent To {email}, Please Enter Otp:"), reply_markup=get_youtube_keyboard())
                        context.user_data["step"] = "otp"
                    else:
                        await update.message.reply_text(fancy(f"Failed To Send Otp: {res['data']}"), reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(fancy("Please Enter 6-Digit Security Code:"), reply_markup=get_youtube_keyboard())
            ...
            if step == "otp":
                ...
                if res["ok"]:
                    ...
                    if unr["ok"]:
                        await update.message.reply_text(fancy(f"Unbind Success\n{unr['data']}"), reply_markup=get_youtube_keyboard())
                    else:
                        await update.message.reply_text(fancy(f"Unbind Failed: {unr['data']}"), reply_markup=get_youtube_keyboard())
                    await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
                    context.user_data.clear()
                else:
        ...
        if flow == "change":
            if step == "method":
                if "otp" in text_normal or "email" in text_normal:
                    ...
                    context.user_data["method"] = "sec"
                await update.message.reply_text(fancy("Please Enter Access Token For Change Email:"), reply_markup=get_youtube_keyboard())
                context.user_data["step"] = "token"
                return STATE_INPUT
            if step == "token":
                context.user_data["token"] = text
                ...
                if context.user_data.get("method") == "otp":
                    await update.message.reply_text(fancy(f"Current Email: {old_email}\nSending Otp..."), reply_markup=get_youtube_keyboard())
                    res = send_otp_sync(old_email, text)
                    if res["ok"]:
                        await update.message.reply_text(fancy("Otp Sent To Old Email, Please Enter Otp:"), reply_markup=get_youtube_keyboard())
                        context.user_data["step"] = "old_otp"
                    else:
                        await update.message.reply_text(fancy(f"Failed To Send Otp: {res['data']}"), reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(fancy("Please Enter 6-Digit Security Code For Old Email:"), reply_markup=get_youtube_keyboard())
                    context.user_data["step"] = "sec_code"
                return STATE_INPUT
            if step == "sec_code":
                ...
                if res["ok"]:
                    ...
                    await update.message.reply_text(fancy("Identity Verified! Please Enter New Email:"), reply_markup=get_youtube_keyboard())
            ...
            if step == "old_otp":
                ...
                if res["ok"]:
                    context.user_data["identity_token"] = res["data"].get("identity_token")
                    await update.message.reply_text(fancy("Old Email Verified! Please Enter New Email:"), reply_markup=get_youtube_keyboard())
                    context.user_data["step"] = "new_email"
                else:
                    await update.message.reply_text(fancy(f"Verify Failed: {res['data']}"))
                return STATE_INPUT
            if step == "new_email":
                ...
                res = send_otp_sync(text, context.user_data["token"])
                if res["ok"]:
                    await update.message.reply_text(fancy(f"Otp Sent To New Email {text}, Please Enter Otp:"), reply_markup=get_youtube_keyboard())
                    context.user_data["step"] = "new_otp"
                else:
                    await update.message.reply_text(fancy(f"Failed To Send Otp: {res['data']}"))
                return STATE_INPUT
            if step == "new_otp":
                ...
                if res["ok"] and res["data"].get("verifier_token"):
                    ...
                    if rebind["ok"]:
                        await update.message.reply_text(fancy(f"Change Success\n{rebind['data']}"), reply_markup=get_youtube_keyboard())
                    else:
                        await update.message.reply_text(fancy(f"Change Failed: {rebind['data']}"), reply_markup=get_youtube_keyboard())
                    await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
                    context.user_data.clear()
                else:
                    await update.message.reply_text(fancy(f"Verify Failed: {res.get('data')}"))
                return STATE_INPUT

        if flow == "cancel":
            if step == "token":
                await update.message.reply_text(fancy("Cancelling..."), reply_markup=get_youtube_keyboard())
                res = cancel_request_sync(text)
                if res["ok"]:
                    await update.message.reply_text(fancy("Cancel Success!"), reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(fancy(f"Cancel Failed: {res.get('data')}"), reply_markup=get_youtube_keyboard())
                await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "eat":
            if step == "eat":
                await update.message.reply_text(fancy("Converting EAT..."), reply_markup=get_youtube_keyboard())
                res = eat_to_token_sync(text)
                if res["ok"]:
                    await update.message.reply_text(fancy(f"Token: {res['access_token']}\nAccount: {res['account_id']}\nNickname: {res['nickname']}"), reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(fancy(f"Failed: {res.get('error')}"), reply_markup=get_youtube_keyboard())
                await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "revoke":
            if step == "token":
                await update.message.reply_text(fancy("Revoking..."), reply_markup=get_youtube_keyboard())
                res = revoke_token_sync(text)
                if res["ok"]:
                    await update.message.reply_text(fancy(f"Revoked! Account: {res.get('account_id')}"), reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(fancy(f"Revoke Failed: {res.get('error')}"), reply_markup=get_youtube_keyboard())
                await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "bound":
            if step == "token":
                await update.message.reply_text(fancy("Checking Bound Accounts..."), reply_markup=get_youtube_keyboard())
                ...
                if res["ok"]:
                    ...
                    if not available:
                        ...
                        for pid in available:
                            b_text += f"{PLATFORM_MAP_FULL.get(pid, f'Unknown ({pid})')}\n"
                    await update.message.reply_text(fancy(b_text), reply_markup=get_youtube_keyboard())
                else:
                    await update.message.reply_text(fancy(f"Error: {res.get('error')}"), reply_markup=get_youtube_keyboard())
                await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        if flow == "login_history":
            if step == "token":
                await update.message.reply_text(fancy("Fetching Login History..."), reply_markup=get_youtube_keyboard())
                try:
                    res = fetch_login_history_sync(text)
                    if not res.get("ok"):
                        await update.message.reply_text(fancy(f"Invalid Token: {res.get('error')}"), reply_markup=get_youtube_keyboard())
                    else:
                        ...
                        if not records:
                            ...
                            for i, rec in enumerate(records[:10], 1):
                                ...
                                ram = rec.get('ram',0)
                                body += f"#{i} - {date_str}\nDevice: {dev}\nArch: {arch}\nRAM: {ram} MB\n\n"
                        await update.message.reply_text(fancy(header+body), reply_markup=get_youtube_keyboard())
                except Exception as e:
                    await update.message.reply_text(fancy(f"Error: {str(e)[:300]}"), reply_markup=get_youtube_keyboard())
                await update.message.reply_text(fancy("Main Menu"), reply_markup=get_reply_keyboard())
                context.user_data.clear()
                return STATE_INPUT
        ...
        traceback.print_exc()
        try:
            await update.message.reply_text(fancy("Error Occurred. Please Use /start"), reply_markup=get_reply_keyboard())
        except:
            pass
        context.user_data.clear()
        return STATE_INPUT
...
@flask_app.route('/')
def home():
    return f"Bot Running - Final Perfect All Options Fixed - Fancy Font"
