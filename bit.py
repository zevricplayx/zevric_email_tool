#!/usr/bin/env python3
"""
Zevric Email Tool - Perfect Clean Version
Owner: zevric x play | TG: @just_zevric | YT: zevricxplay
No extra text, green UI, all options working
"""
...
def start_health_server():
    ...
    try:
        ...
        logger.warning(f"Health fail {e}")
...
async def send_main_menu(update):
    ...
    await update.message.reply_text(f"📢 YouTube: @{OWNER_YT}", reply_markup=sub_keyboard())
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📢 Subscribe - {YT_LINK}", reply_markup=sub_keyboard())

async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    txt = (
        "Commands:\n"
        "Check Bind Info - Show email info\n"
        "Eat To Access Token - Convert EAT URL to token\n"
        "Bind Email - Add email\n"
        "Unbind Email - Remove email\n"
        "Change Bind Email - Change old to new\n"
        "Cancel Bind - Cancel pending\n"
        "Revoke Access Token - Logout\n"
        "Owner - My details"
    )
    await update.message.reply_text(txt, reply_markup=main_menu())
    await update.message.reply_text(f"📢 {YT_LINK}", reply_markup=sub_keyboard())

async def owner_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    txt = (
        f"👑 Developer: {OWNER_NAME}\n"
        f"✈️ Telegram: @{OWNER_TG}\n"
        f"🔗 https://t.me/{OWNER_TG}\n"
        f"▶️ YouTube: {OWNER_YT}\n"
        f"🔗 {YT_LINK}"
    )
...
async def cbi_token_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        d = r.json()
        nick = d.get("nickname") or d.get("user_name") or "Unknown"
        acc = d.get("account_id") or d.get("uid") or "Unknown"
        cur = d.get("email","Not bound")
        pend = d.get("pending_email") or d.get("new_email") or "None"
        cd = int(d.get("countdown",0) or 0)
    ...
    try:
        ...
        if "eat=" in txt:
            ...
            if "eat" in qs:
                ...
                # fallback split
                import re
                m = re.search(r"eat=([^&]+)", txt)
                if m:
                    eat_token = m.group(1)
        elif len(txt) > 100 and "&" not in txt and "=" not in txt:
            eat_token = txt
        else:
            eat_token = txt

        if not eat_token:
            await update.message.reply_text("EAT token nahi mila.")
            await send_main_menu(update)
            return ConversationHandler.END

        await update.message.reply_text("Converting EAT to Access Token...")
        ...
        nickname = urllib.parse.unquote(nickname_raw)
        region = params.get("region", ["Unknown"])[0]

        if access_token:
            await update.message.reply_text(
                f"✅ EAT Converted Successfully!\n\n"
                f"Nickname: {nickname}\n"
                f"Account ID: {account_id}\n"
                f"Region: {region}\n\n"
                f"Access Token:\n{access_token}\n\n"
                f"DO NOT SHARE THIS TOKEN!",
                reply_markup=sub_keyboard()
            )
        else:
            # If API didn't return token, show the eat itself as token
            await update.message.reply_text(
                f"Nickname: {nickname}\nAccount ID: {account_id}\nRegion: {region}\n\n"
                f"Access Token not found in redirect, trying raw EAT as token:\n{eat_token}",
                reply_markup=sub_keyboard()
            )
    except Exception as e:
        await update.message.reply_text(f"Error converting EAT: {e}")
    await send_main_menu(update)
    return ConversationHandler.END
...
async def bind_email_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if j.get("error") == "error_ongoing_request":
            await update.message.reply_text("❌ Pending request hai. Pehle Cancel Bind karo.")
            await send_main_menu(update)
            return ConversationHandler.END
...
async def unbind_tok(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        email = ""
    if not email:
        await update.message.reply_text("No bound email found.")
        await send_main_menu(update)
        return ConversationHandler.END
    context.user_data["email"] = email
    url = "https://100067.connect.garena.com/game/account_security/bind:send_otp"
