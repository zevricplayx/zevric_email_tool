#!/usr/bin/env python3
"""
FINAL - No extra zevricxplay after welcome + How To Use + Change Bind fixed + Full fancy
"""
...
{fancy('If you have EAT link, use this to convert to Access Token. Just paste your EAT URL and bot will give you Access Token.')}

{fancy('2. Check Bind Info:')}
{fancy('Send your Access Token, bot will show Nickname, Account ID, Region, Current Email, Pending Email, Countdown.')}

{fancy('3. Bind Email:')}
{fancy('Token -> Email -> OTP -> 6-digit Security Code -> Done. Use for first time email bind.')}

{fancy('4. Change Bind Email:')}
{fancy('Token -> Bot auto finds OLD email -> OTP to OLD email -> Verify -> Enter NEW email -> OTP to NEW email -> Verify -> Rebind Done.')}

{fancy('5. Unbind Email:')}
{fancy('Token -> OTP to current email -> Unbind request.')}

{fancy('6. Cancel Bind:')}
{fancy('If you have pending request, use this to cancel it before new bind.')}

{fancy('7. Revoke Access Token:')}
{fancy('Logout your token securely.')}

{fancy('Note: If OTP not received, check Spam folder.')}
Bot: {BOT_USERNAME}
"""
